...
import { app, markSceneDirty, setHelperVisibility, helperRegistry } from './scene.js';
...
// Volumetric light (god-rays) — per light and per bloom-object
const MAX_VOL_SOURCES  = 4;
let volMaskComposer    = null;  // isola as fontes (luzes/objetos volumétricos) contra preto
let volGodrayPass      = null;  // blur radial reutilizado — uma vez por fonte
let volSourceTargets   = [];    // um render target por fonte ativa (até MAX_VOL_SOURCES)
let _volLastW = 0, _volLastH = 0;
...
const SelectiveBloomMixShader = {
  ...
  fragmentShader: `
    ...
    uniform sampler2D volTex0, volTex1, volTex2, volTex3;
    uniform float volActive;
    uniform int   volCount;
    varying vec2 vUv;
    void main() {
      vec4 base = texture2D(tDiffuse, vUv);
      vec3 add = vec3(0.0);
      if (bloomActive > 0.5) add += texture2D(bloomTexture, vUv).rgb;
      if (volActive > 0.5) {
        if (volCount > 0) add += texture2D(volTex0, vUv).rgb;
        if (volCount > 1) add += texture2D(volTex1, vUv).rgb;
        if (volCount > 2) add += texture2D(volTex2, vUv).rgb;
        if (volCount > 3) add += texture2D(volTex3, vUv).rgb;
      }
...
function ensureComposer() {
  ...
  selectiveMixPass.uniforms.volActive.value   = 0;
...
// ─────────────────────────────────────────────────────────────────────────────
// VOLUMETRIC LIGHT (god-rays) — por luz (com cor própria) e por objeto com bloom
// ─────────────────────────────────────────────────────────────────────────────
// Técnica: "Volumetric Light Scattering" clássica (blur radial em direção à
// posição da fonte na tela), aplicada sobre uma máscara isolada (fonte
// colorida, resto preto — mesmo truque de esconder/mostrar já usado no bloom
// por objeto acima). Cada fonte é processada com seu próprio blur e cor,
// depois somada aditivamente no frame final.

export function getLightVolumetric(group) {
  const v = group?.userData?.volumetric || {};
  return { enabled: !!v.enabled, color: v.color || '#ffe6b3', intensity: v.intensity ?? 1 };
}
export function setLightVolumetricField(group, key, value) {
  if (!group) return;
  group.userData.volumetric = group.userData.volumetric || {};
  group.userData.volumetric[key] = value;
  markSceneDirty();
}

// Pequena esfera invisível que acompanha a luz — serve de "corpo" pra ela
// aparecer na máscara isolada (uma Light do three.js não tem malha própria).
function _ensureVolLightProxy(group) {
  if (group.userData._volProxyMesh) return group.userData._volProxyMesh;
  const geo  = new THREE.SphereGeometry(0.12, 8, 6);
  const mat  = new THREE.MeshBasicMaterial({ color: 0xffffff, fog: false, toneMapped: false });
  const mesh = new THREE.Mesh(geo, mat);
  mesh.visible = false;
  mesh.userData._isVolProxy = true;
  group.add(mesh);
  group.userData._volProxyMesh = mesh;
  helperRegistry.objects.push(mesh); // nunca aparece em export / clean render
  return mesh;
}

function _collectVolumetricSources() {
  if (!app.scene) return [];
  const sources = [];
  app.scene.traverse((o) => {
    if (sources.length >= MAX_VOL_SOURCES) return;
    if (o.userData?.isLightObject && o.userData?.volumetric?.enabled) {
      sources.push({
        mesh: _ensureVolLightProxy(o),
        tint: new THREE.Color(o.userData.volumetric.color || '#ffe6b3'),
        intensity: o.userData.volumetric.intensity ?? 1,
        swapColor: true,
      });
    } else if ((o.isMesh || o.isSkinnedMesh) && o.userData?.bloom?.enabled && o.userData?.bloom?.volumetric) {
      sources.push({ mesh: o, tint: new THREE.Color(0xffffff), intensity: 1, swapColor: false });
    }
  });
  return sources.slice(0, MAX_VOL_SOURCES);
}

const VolGodrayShader = {
  uniforms: {
    tMask:    { value: null },
  ...
  fragmentShader: `
    ...
    void main() {
      vec2 deltaTextCoord = (vUv - lightPos) * (1.0 / float(NUM_SAMPLES)) * density;
      vec2 coord = vUv;
      ...
      for (int i = 0; i < NUM_SAMPLES; i++) {
        ...
        vec3 s = texture2D(tMask, c).rgb;
        accum += s * illuminationDecay * weight;
        illuminationDecay *= decay;
      }
      gl_FragColor = vec4(accum * exposure * tint, 1.0);
    }
  `,
};

function _ensureVolResources() {
  if (volGodrayPass) return;

  volMaskComposer = new EffectComposer(app.renderer);
  volMaskComposer.renderToScreen = false;
  volMaskComposer.addPass(new RenderPass(app.scene, app.camera));

  volGodrayPass = new ShaderPass(VolGodrayShader);
  volGodrayPass.material.transparent = true;
  volGodrayPass.material.depthTest   = false;
  volGodrayPass.material.depthWrite  = false;
}

function _ensureVolSourceTargets(w, h) {
  if (w === _volLastW && h === _volLastH && volSourceTargets.length === MAX_VOL_SOURCES) return;
  _volLastW = w; _volLastH = h;
  for (let i = 0; i < MAX_VOL_SOURCES; i++) {
    volSourceTargets[i]?.dispose();
    volSourceTargets[i] = new THREE.WebGLRenderTarget(w, h, { type: THREE.HalfFloatType });
  }
  volMaskComposer?.setSize(w, h);
}

const _volTmpVec3 = new THREE.Vector3();

// Chamada uma vez por frame (igual _renderSelectiveBloom). Tudo dentro de
// try/catch — se algo falhar aqui, o efeito só some naquele frame, nunca
// derruba o resto do render.
function _renderVolumetricSources() {
  if (!selectiveMixPass) return;
  const sources = _collectVolumetricSources();
  if (!sources.length) {
    selectiveMixPass.uniforms.volActive.value = 0;
    return;
  }
  try {
    ...
    const w = app.renderer.domElement.clientWidth  || window.innerWidth;
    const h = app.renderer.domElement.clientHeight || window.innerHeight;
    _ensureVolSourceTargets(w, h);

    // 1) Máscara: esconde tudo, mostra só as fontes (mesmo truque do bloom por objeto).
    // Fundo forçado pra preto — senão a cor/skybox real da cena vazaria pra dentro
    // da máscara e o blur radial pegaria isso em vez de só as fontes.
    // try/finally aqui é importante: se algo explodir no meio, a cena tem que
    // voltar ao normal (nada pode ficar escondido/trocado por acidente).
    const prevBg = app.scene.background;
    const hidden = [];
    const swapped = [];
    try {
      ...
      app.scene.traverse((o) => {
        if (!(o.isMesh || o.isSkinnedMesh) || !o.visible) return;
        if (!sources.some((s) => s.mesh === o)) { o.visible = false; hidden.push(o); }
      });
      sources.forEach((s) => {
        s.mesh.visible = true;
        if (s.swapColor && s.mesh.material?.color) {
          swapped.push({ mesh: s.mesh, orig: s.mesh.material });
          if (!s.mesh.userData._volTintMat) {
            s.mesh.userData._volTintMat = new THREE.MeshBasicMaterial({ fog: false, toneMapped: false });
          }
          const tinted = s.mesh.userData._volTintMat;
          tinted.color.copy(s.tint);
          s.mesh.material = tinted;
        }
      });
      volMaskComposer.render();
    } finally {
      app.scene.background = prevBg;
      hidden.forEach((o) => { o.visible = true; });
      swapped.forEach(({ mesh, orig }) => { mesh.material = orig; });
    }

    // 2) Uma passada de blur radial por fonte, cada uma no seu próprio target
    let activeCount = 0;
    sources.forEach((s) => {
      const proj = _volTmpVec3.setFromMatrixPosition(s.mesh.matrixWorld).project(app.camera);
      if (proj.z < -1 || proj.z > 1) return; // fora do frustum / atrás da câmera
      volGodrayPass.uniforms.tMask.value    = volMaskComposer.readBuffer.texture;
      volGodrayPass.uniforms.lightPos.value.set((proj.x + 1) / 2, (proj.y + 1) / 2);
      volGodrayPass.uniforms.tint.value.copy(s.tint);
      volGodrayPass.uniforms.exposure.value = 0.4 * (s.intensity ?? 1);
      volGodrayPass.clear = true;
      volGodrayPass.render(app.renderer, volSourceTargets[activeCount], volMaskComposer.readBuffer, 0, false);
      activeCount++;
    });

    selectiveMixPass.uniforms.volActive.value = activeCount > 0 ? 1 : 0;
    selectiveMixPass.uniforms.volCount.value  = activeCount;
    for (let i = 0; i < MAX_VOL_SOURCES; i++) {
      selectiveMixPass.uniforms['volTex' + i].value = i < activeCount ? volSourceTargets[i].texture : null;
    }
  } catch (e) {
    selectiveMixPass.uniforms.volActive.value = 0;
    console.warn('[volumetric] efeito pulado neste frame:', e);
  }
}
