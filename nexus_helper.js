// ==================== NEXUS HELPER v3 ====================
// Melhorias v3:
// ✔ Todos emojis → ícones SVG
// ✔ Nomes sem copyright (sem "Ben 10")
// ✔ Simulação evo: painel compacto com botão fechar
// ✔ Skill panel: abre via botão toggle (não auto-abre)
// ✔ Idle animation complexa + animação ao usar skill
// ✔ IA em todas funções: evolução, transform, mech, fusão
// ✔ Modelos evoluídos complexos e distintos por arquétipo
// ✔ Efeitos SFM com additive blending, glow e screen flash
// ✔ Material do objeto influencia genoma na simulação
// ✔ Vários planetas com visual detalhado
// ✔ Mobile: duplo-toque para multi-seleção
import * as THREE from 'https://unpkg.com/three@0.169.0/build/three.module.js';

// API Key — salvo no localStorage do navegador
let CLAUDE_API_KEY='';try{CLAUDE_API_KEY=localStorage.getItem('nh_claude_key')||'';}catch{}
...
/* Key modal */
#nh-key-modal{position:fixed;inset:0;z-index:9500;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.72);backdrop-filter:blur(6px)}
...
.nh-key-box h3{margin:0 0 8px;font-size:14px;color:#a5b4fc;display:flex;align-items:center;gap:8px}
.nh-key-box p{font-size:11px;color:rgba(255,255,255,.38);margin:0 0 14px;line-height:1.6}
.nh-key-inp{width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(95,127,255,.3);border-radius:8px;padding:9px 12px;color:rgba(255,255,255,.88);font-size:12.5px;font-family:monospace;outline:none;box-sizing:border-box;margin-bottom:10px}
.nh-key-inp:focus{border-color:rgba(95,127,255,.65)}
.nh-key-st{font-size:11px;min-height:16px;margin-bottom:10px;font-family:monospace}
.nh-key-st.ok{color:#4cefac}.nh-key-st.err{color:#ff5252}
.nh-key-row{display:flex;gap:8px}
...
.nh-key-clear{padding:9px 14px;border-radius:8px;border:1px solid rgba(255,80,80,.3);background:rgba(255,80,80,.07);color:#ff7070;font-size:12px;cursor:pointer;transition:all .14s}
...
.nh-key-btn.on{background:rgba(76,239,172,.14);border-color:rgba(76,239,172,.4);color:#4cefac}`;
...
async function callClaude(system,user,maxTok=600){
  if(!CLAUDE_API_KEY){toast('Configure sua API Key — botão cerebro no header.',4000);return '';}
  const body=JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:maxTok,system,messages:[{role:'user',content:user}]});
  const hdrs={'Content-Type':'application/json','x-api-key':CLAUDE_API_KEY,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-calls':'true'};
  try{
    const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:hdrs,body});
    const d=await r.json();
    if(d.error){toast('Erro IA: '+d.error.message,4000);return '';}
    return d.content?.[0]?.text||'';
  }catch(e1){
    try{
      const r2=await fetch('https://corsproxy.io/?url='+encodeURIComponent('https://api.anthropic.com/v1/messages'),{method:'POST',headers:hdrs,body});
      const d2=await r2.json();
      if(d2.error){toast('Erro IA: '+d2.error.message,4000);return '';}
      return d2.content?.[0]?.text||'';
    }catch(e2){toast('Erro de conexão com a IA.',3500);return '';}
  }
}
...
function buildDOM(){
  ...
  <div class="nh-hdr"><div class="nh-hdr-logo">${ico('nexus',18)}</div><h2>Nexus Helper</h2>
    ...
    <button class="nh-key-btn" id="nh-key-btn" title="Configurar API Key">${ico('brain',13)}</button>
  ...
  // Key modal
  const km=document.createElement('div');km.id='nh-key-modal';km.className='hidden';
  km.innerHTML=`<div class="nh-key-box">
    <h3>${ico('brain',16)} API Key — Claude (Anthropic)</h3>
    <p>Cole sua chave abaixo. Salva no navegador, nunca enviada a terceiros.<br>
    Obtenha em: <strong style="color:#a5b4fc">console.anthropic.com → API Keys</strong></p>
    <input class="nh-key-inp" id="nh-key-inp" type="password" placeholder="sk-ant-api03-…" autocomplete="off"/>
    <div class="nh-key-st" id="nh-key-st"></div>
    <div class="nh-key-row">
      <button class="nh-key-save" id="nh-key-save">${ico('skills',13)} Salvar e Testar</button>
...
// ══════════════════════════════════════════════════════════════════
//  EVOLVED FORM — Modelos articulados com animação skeletal
// ══════════════════════════════════════════════════════════════════

// Cria um mesh filho
function _m(geo,mat,par,x=0,y=0,z=0,rx=0,ry=0,rz=0){
  const m=new THREE.Mesh(geo,mat);
  m.position.set(x,y,z);m.rotation.set(rx,ry,rz);
  m.castShadow=m.receiveShadow=true;par.add(m);return m;
}
// Cria um grupo (osso)
function _b(par,x=0,y=0,z=0,rx=0,ry=0,rz=0){
  const g=new THREE.Group();g.position.set(x,y,z);g.rotation.set(rx,ry,rz);par.add(g);return g;
}
// Material padrão
function _mat(col,ei=0.35,rough=0.32,metal=0.45,tr=false,op=1){
  return new THREE.MeshStandardMaterial({color:col,emissive:col,emissiveIntensity:ei,roughness:rough,metalness:metal,transparent:tr,opacity:op,side:tr?THREE.DoubleSide:THREE.FrontSide});
}
// Material brilhante (olhos, glows)
function _glow(col,ei=6){
  return new THREE.MeshStandardMaterial({color:0,emissive:col,emissiveIntensity:ei,roughness:0,transparent:true,opacity:0.9});
}
...
function buildEvolvedForm(center,genome,archetype){
  ...
  const sk={}; // esqueleto: mapa nome→grupo

  if(archetype.traits.bulk)        _buildTitanorak(root,sk,S,col,genome);
  else if(archetype.traits.shell)  _buildFerro(root,sk,S,col,genome);
  else if(archetype.traits.sonic)  _buildEcho(root,sk,S,col,genome);
  else if(archetype.traits.fire)   _buildPyrovex(root,sk,S,col,genome);
  else if(archetype.traits.ice)    _buildGlacius(root,sk,S,col,genome);
  else if(archetype.traits.web)    _buildArachna(root,sk,S,col,genome);
  else if(archetype.traits.cosmic) _buildVoidEntity(root,sk,S,col,genome);
  else                             _buildVenomApex(root,sk,S,col,genome);

  if(genome.traits.includes('bioluminescence')){
    for(let i=0;i<6;i++){
      const a=i/6*Math.PI*2;
      _m(new THREE.SphereGeometry(S*.035,6,6),_glow(0x44ffaa,5),root,
         Math.cos(a)*S*.44,S*.28+Math.sin(a*2)*S*.14,Math.sin(a)*S*.44);
    }
  }

  root.userData.shapeType='evolved';root.userData.isEvolved=true;
  root.userData._S=S;root.userData._sk=sk;root.userData._archetype=archetype;
  root.userData._baseY=center.y;
  _animateCreature(root,sk,archetype,S);
  return root;
}

// ── VENOM APEX ───────────────────────────────────────────────────
function _buildVenomApex(root,sk,S,col,genome){
  const d=Math.max(0,col-0x1a1a1a),ac=0xcc4422;
  const torso=sk.torso=_b(root,0,S*.28,0);
  _m(new THREE.CylinderGeometry(S*.19,S*.25,S*.7,10),_mat(col,.45,.28,.42),torso);
  for(let i=0;i<6;i++) _m(new THREE.ConeGeometry(S*.035,S*(.14+i*.014),5),_mat(ac,1.2,.1,.7),torso,0,S*(.16-i*.1),-S*.22,Math.PI);
  for(let i=0;i<5;i++) _m(new THREE.TorusGeometry(S*.21,S*.016,4,14),_mat(col,.2,.4,.58),torso,0,S*(.06+i*.12),0,Math.PI*.5);
  const neck=sk.neck=_b(torso,0,S*.38,0);
  _m(new THREE.CylinderGeometry(S*.11,S*.16,S*.26,8),_mat(col,.35,.3,.38),neck);
  const head=sk.head=_b(neck,0,S*.2,-S*.03);
  _m(new THREE.BoxGeometry(S*.3,S*.23,S*.36),_mat(col,.45,.27,.42),head);
  _m(new THREE.BoxGeometry(S*.2,S*.14,S*.2),_mat(d,.22,.3,.4),head,0,-S*.03,S*.25);
  const jaw=sk.jaw=_b(head,0,-S*.07,S*.1);
  _m(new THREE.BoxGeometry(S*.18,S*.07,S*.17),_mat(d,.15,.36,.32),jaw);
  for(let i=-1;i<=1;i++) _m(new THREE.ConeGeometry(S*.018,S*.07,4),_mat(0xeeeedd,.5,.1,.8),jaw,i*S*.05,-S*.04,S*.07,Math.PI);
  [[-.11,.04],[.11,.04],[-.06,.1],[.06,.1]].forEach(([ex,ey])=>
    _m(new THREE.SphereGeometry(S*.04,8,8),_glow(0x88ff44,6),head,ex*S,ey*S,S*.19));
  [-1,1].forEach(s=>{
    _m(new THREE.ConeGeometry(S*.04,S*.2,5),_mat(d,.3,.4,.6),head,s*S*.12,S*.1,-S*.04,-0.3,0,s*.35);
    const sh=sk['sh'+s]=_b(torso,s*S*.27,S*.2,0);
    _m(new THREE.SphereGeometry(S*.12,9,9),_mat(col,.3,.36,.46),sh);
    const ua=sk['ua'+s]=_b(sh,s*S*.13,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.072,S*.062,S*.4,8),_mat(col,.3,.36,.4),ua,0,-S*.2,0);
    const el=sk['el'+s]=_b(ua,0,-S*.42,0);
    _m(new THREE.SphereGeometry(S*.075,7,7),_mat(col,.33,.4,.48),el);
    const fa=sk['fa'+s]=_b(el,0,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.055,S*.072,S*.36,8),_mat(col,.3,.36,.4),fa,0,-S*.18,0);
    _m(new THREE.BoxGeometry(S*.013,S*.21,S*.03),_mat(0x999999,1.5,.06,.95),fa,s*S*.08,-S*.11,S*.06);
    const hd=sk['hd'+s]=_b(fa,0,-S*.38,0);
    for(let f=0;f<3;f++) _m(new THREE.ConeGeometry(S*.02,S*.11,5),_mat(0xdddddd,.8,.12,.9),hd,s*S*.04*(f-1),-S*.06,S*.04,-0.4,0,(f-1)*.3);
    const hip=sk['hip'+s]=_b(torso,s*S*.2,-S*.3,0);
    _m(new THREE.SphereGeometry(S*.1,8,8),_mat(col,.3,.36,.46),hip);
    const th=sk['th'+s]=_b(hip,s*S*.04,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.095,S*.08,S*.44,8),_mat(col,.3,.36,.4),th,0,-S*.22,0);
    const kn=sk['kn'+s]=_b(th,0,-S*.46,0);
    _m(new THREE.SphereGeometry(S*.09,7,7),_mat(col,.33,.38,.48),kn);
    _m(new THREE.ConeGeometry(S*.038,S*.15,5),_mat(ac,1.2,.1,.8),kn,s*S*.1,0,S*.08,0,0,s*.5);
    const sh2=sk['sh2'+s]=_b(kn,0,-S*.04,S*.04);
    _m(new THREE.CylinderGeometry(S*.07,S*.09,S*.4,8),_mat(col,.3,.36,.4),sh2,0,-S*.2,0);
    const ft=sk['ft'+s]=_b(sh2,0,-S*.42,S*.07);
    _m(new THREE.BoxGeometry(S*.1,S*.045,S*.28),_mat(col,.2,.4,.46),ft,0,0,S*.06);
    for(let f=0;f<3;f++) _m(new THREE.ConeGeometry(S*.02,S*.09,5),_mat(0xdddddd,.8,.12,.9),ft,(f-1)*S*.055,-S*.04,S*.19,-0.35,0,(f-1)*.25);
  });
  let prev=torso;
  for(let i=0;i<8;i++){
    const seg=sk['tl'+i]=_b(prev,0,0,-S*(.14-i*.007));
    _m(new THREE.SphereGeometry(Math.max(S*(.1-i*.01),S*.03),7,6),_mat(col,.3,.36,.4),seg);
    if(i>1&&i%2===0) _m(new THREE.ConeGeometry(S*.028,S*.09,5),_mat(ac,1.1,.15,.8),seg,0,0,-S*(.1-i*.01)*1.2,Math.PI);
    prev=seg;
  }
  _m(new THREE.ConeGeometry(S*.048,S*.24,6),_mat(ac,2,.06,.88),prev,0,0,-S*.13,Math.PI*.68);
}

// ── TITANORAK ────────────────────────────────────────────────────
function _buildTitanorak(root,sk,S,col,genome){
  const d=Math.max(0,col-0x181818);
  const torso=sk.torso=_b(root,0,S*.26,0);
  _m(new THREE.BoxGeometry(S*.86,S*.8,S*.53),_mat(col,.44,.26,.56),torso);
  _m(new THREE.BoxGeometry(S*.62,S*.27,S*.1),_mat(d,.24,.2,.7),torso,0,S*.19,S*.27);
  _m(new THREE.BoxGeometry(S*.48,S*.2,S*.08),_mat(d,.2,.2,.7),torso,0,-S*.05,S*.28);
  _m(new THREE.BoxGeometry(S*.09,S*.62,S*.06),new THREE.MeshStandardMaterial({color:0x002244,emissive:0x0088ff,emissiveIntensity:3,roughness:0,metalness:.9,transparent:true,opacity:.88}),torso,0,S*.03,-S*.28);
  for(let i=0;i<4;i++) _m(new THREE.SphereGeometry(S*.053,7,7),_glow(0x0088ff,5),torso,0,S*(.2-i*.18),-S*.32);
  const neck=sk.neck=_b(torso,0,S*.42,0);
  _m(new THREE.CylinderGeometry(S*.21,S*.26,S*.28,10),_mat(col,.4,.28,.52),neck);
  const head=sk.head=_b(neck,0,S*.2,0);
  _m(new THREE.BoxGeometry(S*.44,S*.37,S*.43),_mat(d,.3,.33,.54),head);
  _m(new THREE.BoxGeometry(S*.44,S*.065,S*.08),_mat(col,.22,.38,.62),head,0,S*.19,S*.22);
  [[-0.15,0.16],[0.15,0.16],[-0.09,0.17],[0.09,0.17]].forEach(([hx,hy],i)=>
    _m(new THREE.ConeGeometry(S*(.05-i*.004),S*(.24-i*.025),6),_mat(d,.4,.38,.66),head,hx*S,hy*S,0,-(0.2+i*.08),0,hx*1.6));
  [-1,1].forEach(s=>{
    _m(new THREE.SphereGeometry(S*.062,9,9),_glow(0xff2200,7),head,s*S*.13,S*.04,S*.22);
    _m(new THREE.TorusGeometry(S*.076,S*.011,5,14),_mat(d,.3,.36,.7),head,s*S*.13,S*.04,S*.21,Math.PI*.5);
  });
  for(let i=0;i<4;i++) _m(new THREE.BoxGeometry(S*.28,S*.017,S*.04),_mat(d,.1,.5,.42),head,0,S*(-.06+i*.027),S*.23);
  [-1,1].forEach(s=>{
    const sh=sk['sh'+s]=_b(torso,s*S*.5,S*.26,0);
    _m(new THREE.SphereGeometry(S*.21,10,10),_mat(col,.34,.3,.52),sh);
    for(let i=0;i<3;i++) _m(new THREE.ConeGeometry(S*.042,S*.17,6),_mat(d,.3,.4,.66),sh,s*S*(.05+i*.08),S*(.1-i*.08),0,0,0,s*(.28+i*.24));
    const ua=sk['ua'+s]=_b(sh,s*S*.11,-S*.05,0);
    _m(new THREE.CylinderGeometry(S*.16,S*.13,S*.5,9),_mat(col,.32,.3,.52),ua,0,-S*.25,0);
    const el=sk['el'+s]=_b(ua,0,-S*.52,0);
    _m(new THREE.SphereGeometry(S*.16,8,8),_mat(col,.34,.36,.52),el);
    const fa=sk['fa'+s]=_b(el,s*S*.03,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.14,S*.16,S*.46,9),_mat(col,.3,.3,.52),fa,0,-S*.23,0);
    const fist=sk['fist'+s]=_b(fa,0,-S*.48,0);
    _m(new THREE.BoxGeometry(S*.28,S*.26,S*.23),_mat(col,.32,.3,.56),fist);
    for(let k=0;k<4;k++) _m(new THREE.BoxGeometry(S*.052,S*.042,S*.13),_mat(d,.3,.4,.66),fist,(k-1.5)*S*.066,S*.12,S*.1);
    const hip=sk['hip'+s]=_b(torso,s*S*.27,-S*.4,0);
    const th=sk['th'+s]=_b(hip,0,-S*.02,0);
    _m(new THREE.CylinderGeometry(S*.18,S*.15,S*.52,9),_mat(col,.3,.32,.52),th,0,-S*.26,0);
    const kn=sk['kn'+s]=_b(th,0,-S*.54,0);
    _m(new THREE.SphereGeometry(S*.17,8,8),_mat(col,.32,.36,.52),kn);
    _m(new THREE.ConeGeometry(S*.058,S*.19,6),_mat(d,.3,.4,.66),kn,s*S*.17,0,S*.1,0,0,s*.5);
    const sh2=sk['sh2'+s]=_b(kn,0,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.14,S*.18,S*.48,9),_mat(col,.3,.32,.52),sh2,0,-S*.24,0);
    const ft=sk['ft'+s]=_b(sh2,s*S*.02,-S*.5,S*.06);
    _m(new THREE.BoxGeometry(S*.28,S*.09,S*.42),_mat(col,.22,.38,.52),ft);
  });
}

// ── PYROVEX (dragão) ─────────────────────────────────────────────
function _buildPyrovex(root,sk,S,col,genome){
  const d=Math.max(0,col-0x181818);
  const torso=sk.torso=_b(root,0,S*.18,S*.05);
  _m(new THREE.CylinderGeometry(S*.23,S*.35,S*.83,10),_mat(col,.52,col,.3,.36),torso,0,0,0,.24);
  for(let i=0;i<5;i++) _m(new THREE.TorusGeometry(S*(.25+i*.01),S*.015,4,14),_mat(col,.2,.4,.56),torso,0,S*(.18-i*.14),0,Math.PI*.5);
  for(let i=0;i<4;i++){const a=i/4*Math.PI*2;_m(new THREE.CylinderGeometry(S*.055,S*.072,S*.04,8),_glow(0xff4400,6),torso,Math.cos(a)*S*.27,S*.08,Math.sin(a)*S*.27);}
  // pescoço em segmentos
  let pn=torso;
  for(let i=0;i<5;i++){
    const ns=sk['nk'+i]=_b(pn,0,S*.16,S*(.035-i*.018));
    _m(new THREE.CylinderGeometry(S*(.15-i*.035*.1),S*(.17-i*.03*.1),S*.19,8),_mat(col,.4,col,.3,.36),ns,0,S*.095,0);
    pn=ns;
  }
  const head=sk.head=_b(pn,0,S*.11,S*.06);
  _m(new THREE.BoxGeometry(S*.36,S*.26,S*.45),_mat(col,.5,col,.27,.38),head);
  _m(new THREE.BoxGeometry(S*.25,S*.17,S*.28),_mat(d,.3,d,.3,.4),head,0,-S*.04,S*.34);
  for(let i=0;i<5;i++){const a=(i/4-.5)*1.0;const cr=sk['cr'+i]=_b(head,Math.sin(a)*S*.15,S*.16,0);_m(new THREE.ConeGeometry(S*.046,S*(.19+i*.038),6),_mat(0xff4400,3,.06,.6,true,.9),cr,0,S*.11,0,a*.5);}
  [-1,1].forEach(s=>_m(new THREE.SphereGeometry(S*.06,8,8),_glow(0xff6600,8),head,s*S*.14,S*.04,S*.23));
  const mouth=sk.mouth=_b(head,0,-S*.08,S*.31);
  for(let i=-2;i<=2;i++) _m(new THREE.ConeGeometry(S*.024,S*.088,5),_mat(0xeeeedd,.6,.1,.8),mouth,i*S*.048,-S*.04,S*.02,Math.PI);
  [-1,1].forEach(s=>{
    const wing=sk['wg'+s]=_b(torso,s*S*.28,S*.18,0);
    _m(new THREE.CylinderGeometry(S*.038,S*.058,S*.82,6),_mat(d,.2,.4,.58),wing,s*S*.41,S*.05,0,0,0,s*.7);
    for(let f=0;f<3;f++){const ws=sk['wm'+s+f]=_b(wing,s*S*(.2+f*.2),S*(.06-f*.055),0);_m(new THREE.PlaneGeometry(S*(.53-f*.055),S*(.66-f*.075)),_mat(col,.55+f*.35,0xff2200,.35,.1,true,.52-f*.04),ws,0,-S*.1,0,0,0,s*(.5+f*.18));}
  });
  [-1,1].forEach(s=>[-1,1].forEach(fwd=>{
    const hip=sk['hip'+s+fwd]=_b(torso,s*S*.28,-S*.1,fwd*S*.23);
    const th=sk['th'+s+fwd]=_b(hip,s*S*.055,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.088,S*.077,S*.36,8),_mat(col,.3,d,.36,.4),th,0,-S*.18,0);
    const kn=sk['kn'+s+fwd]=_b(th,0,-S*.38,fwd*S*.04);
    const sh=_b(kn,0,-S*.03,0);
    _m(new THREE.CylinderGeometry(S*.068,S*.088,S*.32,8),_mat(col,.3,d,.36,.4),sh,0,-S*.16,0);
    const ft=_b(sh,0,-S*.34,fwd*S*.055);
    _m(new THREE.BoxGeometry(S*.1,S*.048,S*.27),_mat(col,.2,d,.4,.46),ft,0,0,S*.055);
  }));
  let pt=torso;
  for(let i=0;i<9;i++){const seg=sk['tl'+i]=_b(pt,0,0,-S*(.14-i*.007));_m(new THREE.SphereGeometry(Math.max(S*(.17-i*.015),S*.04),8,6),_mat(col,.34,col,.32,.38),seg);pt=seg;}
  _m(new THREE.ConeGeometry(S*.058,S*.27,6),_mat(0xff4400,2.5,0xff2200,.06,.8),pt,0,0,-S*.14,Math.PI*.64);
}

// ── GLACIUS REX ──────────────────────────────────────────────────
function _buildGlacius(root,sk,S,col,genome){
  const ice=()=>new THREE.MeshStandardMaterial({color:0xbbddff,emissive:0x88ccff,emissiveIntensity:.6,roughness:.02,metalness:.88,transparent:true,opacity:.68});
  const torso=sk.torso=_b(root,0,S*.26,0);
  _m(new THREE.BoxGeometry(S*.54,S*.76,S*.44),_mat(col,.34,col,.18,.72),torso);
  for(let i=0;i<4;i++) _m(new THREE.BoxGeometry(S*.17,S*.5,S*.075),ice(),torso,[- S*.29,S*.29,-S*.27,S*.27][i],S*.03,S*(.26-i*.04));
  for(let i=0;i<7;i++){const a=(i/6-.5)*1.25;const sp=sk['sp'+i]=_b(torso,Math.sin(a)*S*.19,S*.4,0);_m(new THREE.ConeGeometry(S*.036,S*(.27+i*.018),6),ice(),sp,0,S*.15,0,a*.5);}
  const neck=sk.neck=_b(torso,0,S*.4,0);
  _m(new THREE.CylinderGeometry(S*.13,S*.17,S*.27,9),_mat(col,.37,col,.22,.67),neck);
  const head=sk.head=_b(neck,0,S*.19,0);
  _m(new THREE.BoxGeometry(S*.4,S*.33,S*.38),_mat(col,.42,col,.2,.67),head);
  for(let i=0;i<5;i++){const a=(i/4-.5)*1.18;_m(new THREE.ConeGeometry(S*.036,S*(.28+i*.038),6),ice(),head,Math.sin(a)*S*.15,S*.2,0,a*.44);}
  _m(new THREE.TorusGeometry(S*.088,S*.021,6,14),_glow(0x88eeff,4),head,0,0,S*.21,Math.PI*.5);
  [-1,1].forEach(s=>_m(new THREE.SphereGeometry(S*.054,8,8),_glow(0x44ddff,9),head,s*S*.13,S*.06,S*.2));
  [-1,1].forEach(s=>{
    const sh=sk['sh'+s]=_b(torso,s*S*.34,S*.26,0);
    _m(new THREE.SphereGeometry(S*.13,9,9),_mat(col,.3,col,.22,.67),sh);
    const ua=sk['ua'+s]=_b(sh,s*S*.1,-S*.05,0);
    _m(new THREE.CylinderGeometry(S*.095,S*.082,S*.42,8),_mat(col,.3,col,.22,.67),ua,0,-S*.21,0);
    const el=sk['el'+s]=_b(ua,0,-S*.44,0);
    _m(new THREE.SphereGeometry(S*.095,7,7),_mat(col,.32,col,.24,.67),el);
    const fa=sk['fa'+s]=_b(el,0,-S*.03,0);
    _m(new THREE.CylinderGeometry(S*.086,S*.096,S*.4,8),_mat(col,.3,col,.22,.67),fa,0,-S*.2,0);
    _m(new THREE.BoxGeometry(S*.21,S*.19,S*.17),ice(),fa,0,-S*.42,0);
    for(let k=0;k<4;k++) _m(new THREE.ConeGeometry(S*.028,S*(.11+k*.018),5),ice(),fa,(k-1.5)*S*.056,-S*.54,S*.04,-0.22);
    const hip=sk['hip'+s]=_b(torso,s*S*.21,-S*.38,0);
    const th=sk['th'+s]=_b(hip,0,-S*.02,0);
    _m(new THREE.CylinderGeometry(S*.125,S*.105,S*.48,8),_mat(col,.3,col,.22,.67),th,0,-S*.24,0);
    const kn=sk['kn'+s]=_b(th,0,-S*.5,0);
    _m(new THREE.SphereGeometry(S*.115,7,7),_mat(col,.32,col,.25,.67),kn);
    _m(new THREE.ConeGeometry(S*.038,S*.15,5),ice(),kn,s*S*.11,0,S*.08,0,0,s*.5);
    const sh2=sk['sh2'+s]=_b(kn,0,-S*.04,0);
    _m(new THREE.CylinderGeometry(S*.095,S*.125,S*.44,8),_mat(col,.3,col,.22,.67),sh2,0,-S*.22,0);
    _m(new THREE.BoxGeometry(S*.13,S*.31,S*.08),ice(),sh2,0,-S*.19,S*.13);
    const ft=sk['ft'+s]=_b(sh2,s*S*.02,-S*.46,S*.05);
    _m(new THREE.BoxGeometry(S*.26,S*.077,S*.38),_mat(col,.22,col,.24,.67),ft,0,0,S*.055);
  });
}

// ── ECHO SPECTER ─────────────────────────────────────────────────
function _buildEcho(root,sk,S,col,genome){
  const d=Math.max(0,col-0x181818);
  const torso=sk.torso=_b(root,0,S*.26,0);
  _m(new THREE.CylinderGeometry(S*.1,S*.14,S*.83,8),_mat(col,.44,col,.42,.22),torso);
  for(let i=0;i<5;i++) _m(new THREE.TorusGeometry(S*.12,S*.013,5,14),_glow(col,5),torso,0,S*(.18-i*.14),0,Math.PI*.5);
  _m(new THREE.SphereGeometry(S*.08,8,8),_glow(col,7,true,.72),torso,0,S*.03,0);
  const neck=sk.neck=_b(torso,0,S*.43,0);
  _m(new THREE.CylinderGeometry(S*.066,S*.096,S*.3,7),_mat(col,.37,d,.4,.22),neck);
  const head=sk.head=_b(neck,0,S*.19,0);
  _m(new THREE.SphereGeometry(S*.2,10,8),_mat(col,.42,d,.38,.22),head);
  [-1,1].forEach(s=>{
    const ear=sk['ear'+s]=_b(head,s*S*.21,S*.07,0);
    _m(new THREE.ConeGeometry(S*.057,S*.5,6),_mat(col,.3,d,.4,.22),ear,0,S*.25,0,0,0,s*.55);
    _m(new THREE.PlaneGeometry(S*.28,S*.44),_mat(col,.42,col,.4,.1,true,.48),ear,s*S*.05,S*.04,0,0,0,s*.5);
    for(let v=0;v<3;v++) _m(new THREE.CylinderGeometry(S*.005,S*.005,S*.28,3),_mat(col,1.5,col,.5,.1),ear,s*S*(.035+v*.035),S*.05,.01,0,0,s*(.38+v*.2));
  });
  [-1,1].forEach(s=>_m(new THREE.SphereGeometry(S*.054,8,8),_glow(0x00ffff,9),head,s*S*.11,S*.02,S*.2));
  for(let i=0;i<3;i++) _m(new THREE.TorusGeometry(S*.052,S*.017,5,12),_glow(0x4488ff,4.5),head,0,S*(-.02+i*.055),S*.2,Math.PI*.5);
  [-1,1].forEach(s=>{
    const sh=sk['sh'+s]=_b(torso,s*S*.19,S*.24,0);
    const ua=sk['ua'+s]=_b(sh,s*S*.04,0,0);
    _m(new THREE.CylinderGeometry(S*.033,S*.048,S*.48,6),_mat(col,.3,d),ua,0,-S*.24,0);
    const el=sk['el'+s]=_b(ua,0,-S*.5,0);
    const fa=sk['fa'+s]=_b(el,0,-S*.03,0);
    _m(new THREE.CylinderGeometry(S*.026,S*.034,S*.44,6),_mat(col,.28,d),fa,0,-S*.22,0);
    for(let f=0;f<3;f++){const fin=sk['fin'+s+f]=_b(fa,s*S*.055,-S*(.07+f*.11),0);_m(new THREE.PlaneGeometry(S*.15,S*.27),_mat(col,.55,col,.42,.1,true,.44),fin,0,0,0,0,f*.2,s*.46);}
    const hd=_b(fa,0,-S*.46,0);
    for(let f=0;f<4;f++) _m(new THREE.CylinderGeometry(S*.015,S*.021,S*.17,5),_mat(col,.25,d),hd,(f-1.5)*S*.038,-S*.085,0);
    const hip=sk['hip'+s]=_b(torso,s*S*.12,-S*.42,0);
    const th=sk['th'+s]=_b(hip,0,-S*.02,0);
    _m(new THREE.CylinderGeometry(S*.044,S*.035,S*.54,6),_mat(col,.24,d),th,0,-S*.27,0);
    const kn=sk['kn'+s]=_b(th,0,-S*.56,0);
    _m(new THREE.SphereGeometry(S*.052,6,6),_mat(col,.27,d),kn);
    const sh2=sk['sh2'+s]=_b(kn,0,-S*.03,0);
    _m(new THREE.CylinderGeometry(S*.034,S*.044,S*.4,6),_mat(col,.22,d),sh2,0,-S*.2,0);
    const ft=_b(sh2,0,-S*.42,0);
    _m(new THREE.CylinderGeometry(S*.1,S*.088,S*.04,8),_mat(col,.2,d),ft);
  });
}

// ── ARACHNA PRIME ────────────────────────────────────────────────
function _buildArachna(root,sk,S,col,genome){
  const d=Math.max(0,col-0x181818);
  const abd=sk.abd=_b(root,0,S*.06,-S*.3);
  _m(new THREE.SphereGeometry(S*.42,12,10),_mat(col,.46,col,.28,.44),abd);
  for(let i=0;i<4;i++) _m(new THREE.TorusGeometry(S*(.09+i*.077),S*.01,4,20),_mat(0xdddddd,.5,0x888888,.4,.3),abd,0,0,0,Math.PI*.5);
  for(let i=0;i<3;i++) _m(new THREE.CylinderGeometry(S*.037,S*.054,S*.12,8),_mat(col,.3,d,.32,.4),abd,(i-1)*S*.077,-S*.14,-S*.41,.6);
  const ceph=sk.ceph=_b(root,0,S*.24,S*.13);
  _m(new THREE.SphereGeometry(S*.3,10,8),_mat(col,.43,d,.32,.46),ceph);
  [[-0.19,.12],[.19,.12],[-.1,.21],[.1,.21],[-.21,.02],[.21,.02],[-.06,.06],[.06,.06]].forEach(([ex,ey])=>
    _m(new THREE.SphereGeometry(S*.036,6,6),_glow(0x22ff88,6),ceph,ex*S,ey*S,S*.3));
  [-1,1].forEach(s=>{
    const ch=sk['ch'+s]=_b(ceph,s*S*.1,-S*.055,S*.29);
    _m(new THREE.CylinderGeometry(S*.048,S*.067,S*.21,6),_mat(d,.2,d,.4,.5),ch,0,-S*.105,0,.48);
    _m(new THREE.ConeGeometry(S*.053,S*.19,6),_mat(0xaaaaaa,1.5,0x444444,.06,.93),ch,0,-S*.21,S*.06,-0.28);
  });
  for(let i=0;i<8;i++){
    const side=i<4?-1:1,li=i%4;
    const ph=li/4-.5;
    const cx=sk['cx'+i]=_b(ceph,side*S*.28,S*(.03-li*.04),li*S*.056-S*.077);
    _m(new THREE.CylinderGeometry(S*.037,S*.052,S*.4,6),_mat(col,.27,d),cx,side*S*.15,0,0,0,0,side*(.76+li*.17));
    const tr=sk['tr'+i]=_b(cx,side*S*(.3+li*.038),S*(.038-li*.038),0);
    _m(new THREE.SphereGeometry(S*.052,6,6),_mat(col,.3,d),tr);
    const fm=sk['fm'+i]=_b(tr,0,0,0);
    _m(new THREE.CylinderGeometry(S*.031,S*.04,S*.48,6),_mat(col,.24,d),fm,side*S*.15,-S*(.055+li*.038),0,0,0,side*(1.1+li*.1));
    const ti=sk['ti'+i]=_b(fm,side*S*(.28+li*.038),-S*(.095+li*.056),0);
    _m(new THREE.CylinderGeometry(S*.024,S*.031,S*.5,6),_mat(col,.21,d),ti,side*S*.13,-S*(.11+li*.038),0,0,0,side*(1.3+li*.1));
    const ta=sk['ta'+i]=_b(ti,side*S*(.22+li*.038),-S*(.21+li*.038),0);
    _m(new THREE.ConeGeometry(S*.022,S*.096,5),_mat(0xcccccc,1.5,0x555555,.06,.93),ta,0,-S*.038,0,0,0,side*Math.PI*.5);
  }
}

// ── VOID ENTITY ──────────────────────────────────────────────────
function _buildVoidEntity(root,sk,S,col,genome){
  _m(new THREE.SphereGeometry(S*.42,16,14),new THREE.MeshStandardMaterial({color:0x000022,emissive:0x2200aa,emissiveIntensity:1.5,roughness:0,transparent:true,opacity:.53,side:THREE.DoubleSide}),root,0,S*.28,0);
  _m(new THREE.SphereGeometry(S*.24,12,10),_glow(0x5500cc,3.5,true,.78),root,0,S*.28,0);
  _m(new THREE.TorusGeometry(S*.36,S*.038,8,40),_glow(0x8800ff,5),root,0,S*.28,0);
  _m(new THREE.TorusGeometry(S*.28,S*.023,6,32),_glow(0xaa44ff,3.5,true,.73),root,0,S*.28,0,Math.PI*.5,0,.8);
  for(let t=0;t<6;t++){
    const ta=t/6*Math.PI*2;
    const tent=sk['tent'+t]=_b(root,Math.cos(ta)*S*.14,S*.28,Math.sin(ta)*S*.14);
    for(let s=0;s<6;s++){
      const st=s/5;
      const seg=sk['ts'+t+'_'+s]=_b(tent,Math.cos(ta)*S*st*.36,-st*S*.2+Math.sin(st*3)*S*.077,Math.sin(ta)*S*st*.36);
      _m(new THREE.SphereGeometry(S*(.057-st*.04),5,5),_glow(0x6600cc,2+st*2,true,.72-st*.11),seg);
    }
  }
  const orbColors=[0x8800ff,0x4400cc,0xaa44ff,0x660099,0x9933ff,0x5500aa];
  const frags=[];
  for(let i=0;i<12;i++){
    const a=i/12*Math.PI*2,r=S*(.5+(i%3)*.11);
    const frag=new THREE.Mesh(new THREE.TetrahedronGeometry(S*(.038+(i%4)*.014)),_glow(orbColors[i%6],4+(i%3),true,.86));
    frag.position.set(Math.cos(a)*r,S*.28+Math.sin(i*.78)*S*.17,Math.sin(a)*r);
    frag.userData.orbitR=r;frag.userData.orbitA=a;frag.userData.orbitSpeed=.008+(i%5)*.003;
    frag.userData.orbitY=frag.position.y;frag.userData.orbitPh=i*.6;
    root.add(frag);frags.push(frag);sk['orb'+i]=frag;
  }
  root.userData.orbitFrags=frags;
  for(let i=0;i<3;i++){const a=i/3*Math.PI*2;_m(new THREE.SphereGeometry(S*.04,7,7),_glow(0xffffff,10),root,Math.cos(a)*S*.27,S*.32,Math.sin(a)*S*.27);}
}

// helper for glow mat with transparency param


// ══════════════════════════════════════════════════════════════════
//  ANIMAÇÃO SKELETAL — loop por criatura
// ══════════════════════════════════════════════════════════════════
function _animateCreature(root,sk,archetype,S){
  const baseY=root.userData._baseY||root.position.y;
  let f=0;
  const frags=root.userData.orbitFrags;
  const anim=()=>{
    if(!root.parent)return;f++;
    // flutuação
    root.position.y=baseY+Math.sin(f*.018)*.13;
    // pulso olhos
    root.traverse(c=>{if(c.isMesh&&c.material?.emissiveIntensity>3&&(c.material.color?.r??1)<.05){if(!c._ei0)c._ei0=c.material.emissiveIntensity;c.material.emissiveIntensity=c._ei0+Math.sin(f*.15+c.id*.3)*2.2;}});
    // fragmentos void
    if(frags)frags.forEach(fr=>{fr.userData.orbitA+=fr.userData.orbitSpeed;fr.position.x=Math.cos(fr.userData.orbitA)*fr.userData.orbitR;fr.position.z=Math.sin(fr.userData.orbitA)*fr.userData.orbitR;fr.position.y=fr.userData.orbitY+Math.sin(f*.027+fr.userData.orbitPh)*S*.13;fr.rotation.x+=.04;fr.rotation.y+=.06;});

    // caminhar bípede genérico
    const walk=(speed,amp)=>{
      [-1,1].forEach(s=>{
        const ph=s*Math.PI;
        if(sk['th'+s]) sk['th'+s].rotation.x=Math.sin(f*speed+ph)*amp;
        if(sk['sh2'+s]) sk['sh2'+s].rotation.x=Math.abs(Math.sin(f*speed+ph))*amp*1.4;
        if(sk['ua'+s]) sk['ua'+s].rotation.x=Math.sin(f*speed-ph)*amp*.9;
        if(sk['fa'+s]) sk['fa'+s].rotation.x=Math.sin(f*speed-ph)*amp*.6;
      });
      if(sk.neck)sk.neck.rotation.z=Math.sin(f*speed*.6)*.05;
      if(sk.head)sk.head.rotation.y=Math.sin(f*speed*.5)*.09;
    };

    if(archetype.traits.bulk)  walk(.023,.16);
    else if(archetype.traits.ice)   walk(.034,.2);
    else if(archetype.traits.sonic) walk(.044,.24);
    else if(!archetype.traits.web&&!archetype.traits.cosmic) walk(.042,.21);

    // mandíbula (venom)
    if(sk.jaw) sk.jaw.rotation.x=Math.max(0,Math.sin(f*.028)*.24);

    // cauda
    for(let i=0;i<9;i++)if(sk['tl'+i]){sk['tl'+i].rotation.y=Math.sin(f*.05+i*.55)*(0.17+i*.02);sk['tl'+i].rotation.x=Math.sin(f*.04+i*.4)*.09;}

    // asas (pyro)
    if(archetype.traits.fire){
      [-1,1].forEach(s=>{if(sk['wg'+s]){sk['wg'+s].rotation.z=s*(.28+Math.sin(f*.05)*.24);sk['wg'+s].rotation.x=Math.sin(f*.05+s)*.11;}});
      for(let i=0;i<5;i++)if(sk['cr'+i]){sk['cr'+i].rotation.z=Math.sin(f*.08+i*.8)*.11;sk['cr'+i].scale.y=.9+Math.sin(f*.1+i)*.14;}
      for(let i=0;i<5;i++)if(sk['nk'+i]){sk['nk'+i].rotation.y=Math.sin(f*.032+i*.68)*.11;sk['nk'+i].rotation.x=Math.sin(f*.026+i*.5)*.077;}
      if(sk.mouth)sk.mouth.rotation.x=Math.max(0,Math.sin(f*.022)*.28);
    }

    // espículas de gelo
    if(archetype.traits.ice)for(let i=0;i<7;i++)if(sk['sp'+i])sk['sp'+i].scale.y=.87+Math.sin(f*.072+i*.6)*.14;

    // 8 patas aranha
    if(archetype.traits.web){
      for(let i=0;i<8;i++){const ph=i/8*Math.PI*2,side=i<4?1:-1;if(sk['fm'+i])sk['fm'+i].rotation.z=Math.sin(f*.062+ph)*.19*side;if(sk['ti'+i])sk['ti'+i].rotation.z=Math.sin(f*.062+ph+.5)*.27*side;if(sk['ta'+i])sk['ta'+i].rotation.z=Math.sin(f*.062+ph+1)*.17*side;}
      [-1,1].forEach(s=>{if(sk['ch'+s])sk['ch'+s].rotation.x=Math.sin(f*.08)*.19;});
      if(sk.abd){sk.abd.scale.x=1+Math.sin(f*.032)*.038;sk.abd.scale.z=1+Math.sin(f*.032)*.038;}
    }

    // tentáculos void
    if(archetype.traits.cosmic){
      for(let t=0;t<6;t++){if(sk['tent'+t]){sk['tent'+t].rotation.y=Math.sin(f*.022+t*1.18)*.17;sk['tent'+t].rotation.x=Math.sin(f*.018+t*.9)*.11;}}
    }

    // orelhas echo
    if(archetype.traits.sonic){
      [-1,1].forEach(s=>{if(sk['ear'+s])sk['ear'+s].rotation.z=s*(.38+Math.sin(f*.062)*.11);for(let fv=0;fv<3;fv++)if(sk['fin'+s+fv]){sk['fin'+s+fv].scale.y=1+Math.sin(f*.1+fv*1.2)*.09;sk['fin'+s+fv].rotation.y=Math.sin(f*.08+fv*.8)*.11;}});
    }

    // pinças ferro
    if(archetype.traits.shell){
      [-1,1].forEach(s=>{const open=Math.abs(Math.sin(f*.042));if(sk['ch'+s])sk['ch'+s].rotation.y=s*(-.25-open*.48);});
      for(let i=0;i<6;i++){const ph=i/6*Math.PI*2;if(sk['cx'+i])sk['cx'+i].rotation.z=Math.sin(f*.052+ph)*.19;if(sk['tr'+i])sk['tr'+i].rotation.z=Math.sin(f*.052+ph+.66)*.24;}
    }
...
function startIdleAnim(group,archetype){
  // compatibilidade — _animateCreature já é chamado em buildEvolvedForm
}

function triggerSkillAnim(obj){
  const orig=obj.scale.clone();let f=0;
  const a=()=>{f++;if(f<8)obj.scale.setScalar(orig.x*(1+f*.026));else if(f<18)obj.scale.setScalar(orig.x*(1.18-(f-8)*.022));else{obj.scale.copy(orig);markDirty(2);return;}markDirty(2);requestAnimationFrame(a);};
  requestAnimationFrame(a);
  const arch=obj.userData._archetype||obj.userData.archetype;
  screenFlash(arch?.traits?.fire?'rgba(255,100,0,.5)':arch?.traits?.ice?'rgba(100,200,255,.35)':arch?.traits?.cosmic?'rgba(180,0,255,.45)':'rgba(150,120,255,.35)',110);
}
...
// ══════════════════════════════════════════════════════════════════
const SKILL_TMPLS=[
  {id:'strike', name:'Nexal Strike',  icon:'lightning',desc:'Ataque energético',          color:'#5f7fff',eff:'lightning'},
  {id:'shield', name:'Bio Shield',    icon:'shield',   desc:'Barreira orgânica',           color:'#4cefac',eff:'shield'},
...
function _openKeyModal(){
  const m=$('nh-key-modal');if(!m)return;
  const inp=$('nh-key-inp');if(inp)inp.value=CLAUDE_API_KEY||'';
  const st=$('nh-key-st');
  if(st){if(CLAUDE_API_KEY){st.className='nh-key-st ok';st.textContent='Chave salva - pronta para uso';}
         else{st.className='nh-key-st';st.textContent='Nenhuma chave configurada.';}}
  $('nh-key-btn')?.classList.toggle('on',!!CLAUDE_API_KEY);
  m.classList.remove('hidden');}

async function _testKey(key){
  const st=$('nh-key-st');
  if(!key){if(st){st.className='nh-key-st err';st.textContent='Cole a chave antes de salvar.';}return;}
  if(st){st.className='nh-key-st';st.textContent='Testando...';}
  const body=JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:5,messages:[{role:'user',content:'hi'}]});
  const hdrs={'Content-Type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-calls':'true'};
  try{
    let r=null;
    try{r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:hdrs,body});}
    catch{r=await fetch('https://corsproxy.io/?url='+encodeURIComponent('https://api.anthropic.com/v1/messages'),{method:'POST',headers:hdrs,body});}
    const d=await r.json();
    if(d.error){if(st){st.className='nh-key-st err';st.textContent='Erro: '+d.error.message;}return;}
    CLAUDE_API_KEY=key;try{localStorage.setItem('nh_claude_key',key);}catch{}
    if(st){st.className='nh-key-st ok';st.textContent='Chave valida! IA ativada.';}
    $('nh-key-btn')?.classList.add('on');
    setTimeout(()=>$('nh-key-modal')?.classList.add('hidden'),1200);
    toast('IA ativada!',2500);
  }catch(e){if(st){st.className='nh-key-st err';st.textContent='Falha de conexao: '+e.message;}}
}
...
function wireEvents(){
  ...
  // Key modal
  $('nh-key-btn')?.addEventListener('click',e=>{e.stopPropagation();_openKeyModal();});
  ...
  $('nh-key-save')?.addEventListener('click',()=>_testKey($('nh-key-inp')?.value.trim()));
  $('nh-key-inp')?.addEventListener('keydown',e=>{if(e.key==='Enter')$('nh-key-save')?.click();});
  $('nh-key-clear')?.addEventListener('click',()=>{CLAUDE_API_KEY='';try{localStorage.removeItem('nh_claude_key');}catch{}const inp=$('nh-key-inp');if(inp)inp.value='';const st=$('nh-key-st');if(st){st.className='nh-key-st';st.textContent='Chave removida.';}$('nh-key-btn')?.classList.remove('on');toast('Chave removida.',2000);});
...
function init(){
  ...
  // Hook active object changed — 3 strategies for reliability
  const prevCb=window.onActiveObjectChanged;
  window.onActiveObjectChanged=obj=>{if(typeof prevCb==='function')prevCb(obj);updateSkillToggle(obj);};

  // Fallback 1: poll every 350ms
  let _lastAO=null;
  setInterval(()=>{const ao=window.activeObject||null;if(ao!==_lastAO){_lastAO=ao;updateSkillToggle(ao);}},350);

  // Fallback 2: canvas click
  setTimeout(()=>{const c=window._nexusRenderer?.domElement;
    if(c)c.addEventListener('pointerup',()=>{setTimeout(()=>updateSkillToggle(window.activeObject||null),100);});
  },1000);

  // Show key button state
  if(CLAUDE_API_KEY)$('nh-key-btn')?.classList.add('on');
  else setTimeout(()=>toast('Configure sua API Key da IA — botao cerebro no topo.',5000),1800);
}
