...
// ══════════════════════════════════════════════════════════════════
//  CONFIGURAÇÃO DA IA — cole sua chave aqui ou use o painel
// ══════════════════════════════════════════════════════════════════
let CLAUDE_API_KEY = localStorage.getItem('nh_claude_key') || '';
...
async function callClaude(system,user,maxTok=600){
  if(!CLAUDE_API_KEY){toast("Cole sua API Key no painel — botão chave no header.",4000);return "";}
  try{
    const r=await fetch("https://api.anthropic.com/v1/messages",{
      method:"POST",
      headers:{"Content-Type":"application/json","x-api-key":CLAUDE_API_KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-calls":"true"},
  ...
  }catch(e){toast("Erro de conexão com a IA.",3000);return "";}
}
...
function buildDOM(){
  ...
  <div class="nh-key-box">
    ...
    <p>Cole sua chave da Anthropic abaixo. Ela fica salva no navegador (localStorage) e é usada para evolução, transform, mech e fusão por IA.<br><br>Obtenha em: <strong style="color:#a5b4fc">console.anthropic.com → API Keys</strong></p>
...
function buildEvolvedForm(center,genome,archetype){
  const group=new THREE.Group();group.name=`Evolved_${archetype.name.replace(/ /g,'_')}`;group.position.copy(center);
  const S=.75+genome.size*.38;
  const col=parseInt(archetype.color.replace('#',''),16);
  const dark=Math.max(0,col-0x222222);
  const M=(c,ei=.4,em=null,rough=.3,metal=.45)=>new THREE.MeshStandardMaterial({color:c,emissive:em!==null?em:c,emissiveIntensity:ei,roughness:rough,metalness:metal});
  const P=(geo,mat,x=0,y=0,z=0,rx=0,ry=0,rz=0)=>{const m=new THREE.Mesh(geo,mat);m.position.set(x,y,z);m.rotation.set(rx,ry,rz);m.castShadow=m.receiveShadow=true;group.add(m);return m;};

  // Select builder based on archetype traits
  if(archetype.traits.bulk)        _buildTitanorak(group,S,col,dark,genome,P,M);
  else if(archetype.traits.shell)  _buildFerro(group,S,col,dark,genome,P,M);
  else if(archetype.traits.sonic)  _buildEcho(group,S,col,dark,genome,P,M);
  else if(archetype.traits.fire)   _buildPyro(group,S,col,dark,genome,P,M);
  else if(archetype.traits.ice)    _buildGlacius(group,S,col,dark,genome,P,M);
  else if(archetype.traits.web)    _buildArachna(group,S,col,dark,genome,P,M);
  else if(archetype.traits.cosmic) _buildVoid(group,S,col,dark,genome,P,M);
  else                             _buildVenom(group,S,col,dark,genome,P,M);

  // Universal trait additions
  if(genome.traits.includes('bioluminescence')){
    for(let i=0;i<5;i++){const phi=Math.random()*Math.PI*2,theta=Math.random()*Math.PI;
      const orb=new THREE.Mesh(new THREE.SphereGeometry(S*.04,5,5),new THREE.MeshStandardMaterial({color:0,emissive:0x44ffaa,emissiveIntensity:5,roughness:0}));
      orb.position.set(Math.sin(theta)*Math.cos(phi)*S*.5,Math.sin(theta)*Math.sin(phi)*S*.5+S*.3,Math.cos(theta)*S*.5);group.add(orb);}
  }
  group.userData.shapeType='evolved';group.userData.isEvolved=true;group.userData._S=S;
  return group;
}

function _buildVenom(group,S,col,dark,genome,P,M){
  P(new THREE.CylinderGeometry(S*.18,S*.24,S*.75,10),M(col,.5,col,.25,.4),0,S*.3,0);
  for(let i=0;i<5;i++)P(new THREE.TorusGeometry(S*.22,S*.018,4,14),M(col,.2,dark,.4,.6),0,S*(.08+i*.12),0,Math.PI*.5);
  P(new THREE.CylinderGeometry(S*.13,S*.17,S*.28,8),M(col,.35,dark,.3,.4),0,S*.78,0);
  P(new THREE.BoxGeometry(S*.32,S*.25,S*.38),M(col,.5,dark,.25,.45),0,S*.98,-S*.04);
  P(new THREE.BoxGeometry(S*.22,S*.15,S*.22),M(dark,.2,dark,.3,.4),0,S*.92,S*.18);
  for(let i=0;i<7;i++)P(new THREE.ConeGeometry(S*.035,S*(.15+i*.02),5),M(0xcc4422,1.2,0x551100,.1,.8),0,S*(.1+i*.12),-S*.2,Math.PI);
  [[-.12,-.04],[.12,-.04],[-.07,.07],[.07,.07]].forEach(([ex,ey])=>{const eye=new THREE.Mesh(new THREE.SphereGeometry(S*.045,8,8),new THREE.MeshStandardMaterial({color:0,emissive:0x88ff44,emissiveIntensity:6,roughness:0}));eye.position.set(ex*S,(0.98+ey)*S,S*.2);group.add(eye);});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.07,S*.06,S*.38,8),M(col,.3,dark),s*S*.32,S*.42,0,0,0,s*.5);P(new THREE.CylinderGeometry(S*.055,S*.07,S*.36,8),M(col,.3,dark),s*S*.42,S*.16,0,0,0,s*.75);P(new THREE.BoxGeometry(S*.015,S*.25,S*.035),M(0x999999,1.5,0x446677,.05,.95),s*S*.52,S*.1,S*.05);[-1,0,1].forEach(f=>P(new THREE.ConeGeometry(S*.022,S*.12,5),M(0xdddddd,.8,0x666666,.1,.9),s*(S*.48+f*.06),0,S*.05,-.4,0,f*.3));});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.1,S*.08,S*.38,8),M(col,.3,dark),s*S*.2,S*(-.02),0,0,0,s*.25);P(new THREE.CylinderGeometry(S*.07,S*.1,S*.34,8),M(col,.3,dark),s*S*.23,S*(-.3),0,0,0,s*(-.4));P(new THREE.BoxGeometry(S*.09,S*.04,S*.24),M(col,.2,dark),s*S*.19,S*(-.5),S*.07);});
  for(let i=0;i<10;i++)P(new THREE.SphereGeometry(S*(.12-i*.009),8,6),M(col,.3,dark),0,S*(-.08-i*.14),S*(-0.1-i*.1));
  P(new THREE.ConeGeometry(S*.05,S*.28,6),M(0xcc4422,2,0x441100,.05,.9),0,S*(-.18),S*(-1.35),-.65);
}

function _buildFerro(group,S,col,dark,genome,P,M){
  P(new THREE.SphereGeometry(S*.55,14,12),M(col,.4,col,.2,.7),0,S*.35,0);
  for(let ring=0;ring<2;ring++){const cnt=8-ring*2,r=S*(.58+ring*.08);for(let i=0;i<cnt;i++){const a=i/cnt*Math.PI*2;P(new THREE.BoxGeometry(S*.28,S*.09,S*.22),M(col,.35,dark,.2,.75),Math.cos(a)*r,S*.35+ring*S*.15,Math.sin(a)*r,0,a,0);}}
  for(let i=0;i<6;i++){const a=i/6*Math.PI*2;P(new THREE.BoxGeometry(S*.2,S*.06,S*.16),M(col,.2,dark,.15,.8),Math.cos(a)*S*.3,S*.78,Math.sin(a)*S*.3,0,a,.25);}
  P(new THREE.SphereGeometry(S*.28,10,8),M(dark,.3,dark,.3,.6),0,S*.85,S*.18);
  P(new THREE.BoxGeometry(S*.34,S*.12,S*.06),M(col,.15,dark,.35,.7),0,S*.88,S*.45,-.3);
  [-1,1].forEach(s=>P(new THREE.BoxGeometry(S*.08,S*.04,S*.04),new THREE.MeshStandardMaterial({color:0,emissive:0xff8800,emissiveIntensity:5,roughness:0}),s*S*.12,S*.88,S*.48));
  [-2,-1,0,1,2,3].forEach((i,idx)=>{const side=idx<3?-1:1,ix=idx%3;const lx=side*S*.68,ly=S*(-.05-ix*.18),lz=S*(-.18+ix*.22);P(new THREE.CylinderGeometry(S*.1,S*.09,S*.38,8),M(dark,.2,dark,.4,.6),lx,ly,lz,0,0,side*1.2);P(new THREE.SphereGeometry(S*.12,7,7),M(col,.3,dark,.3,.65),lx+side*S*.2,ly-S*.15,lz);P(new THREE.ConeGeometry(S*.07,S*.18,5),M(0xaaaaaa,1,0x555555,.1,.9),lx+side*S*.4,ly-S*.42,lz,0,0,side*Math.PI*.5);});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.12,S*.09,S*.5,8),M(col,.35,dark,.25,.7),s*S*.45,S*.2,S*.42,0,0,-s*.6);P(new THREE.ConeGeometry(S*.1,S*.35,6),M(0xbbbbbb,1.2,0x444444,.05,.95),s*S*.55,S*.02,S*.68,s*.2,0,-s*.3);P(new THREE.ConeGeometry(S*.1,S*.35,6),M(0xbbbbbb,1.2,0x444444,.05,.95),s*S*.35,S*.12,S*.72,-s*.3,0,s*.4);});
}

function _buildEcho(group,S,col,dark,genome,P,M){
  P(new THREE.CylinderGeometry(S*.12,S*.15,S*.88,8),M(col,.45,col,.4,.2),0,S*.38,0);
  for(let i=0;i<5;i++)P(new THREE.TorusGeometry(S*.14,S*.015,5,14),new THREE.MeshStandardMaterial({color:0,emissive:col,emissiveIntensity:5,roughness:0}),0,S*(.1+i*.14),0,Math.PI*.5);
  P(new THREE.SphereGeometry(S*.1,8,8),new THREE.MeshStandardMaterial({color:0,emissive:col,emissiveIntensity:6,roughness:0,transparent:true,opacity:.7}),0,S*.35,0);
  P(new THREE.CylinderGeometry(S*.08,S*.11,S*.36,8),M(col,.35,dark,.4,.2),0,S*.88,0);
  P(new THREE.SphereGeometry(S*.22,10,8),M(col,.4,dark,.35,.2),0,S*1.07,0);
  [-1,1].forEach(s=>{P(new THREE.ConeGeometry(S*.06,S*.55,6),M(col,.3,dark,.4,.2),s*S*.22,S*1.18,0,0,0,s*.55);P(new THREE.PlaneGeometry(S*.32,S*.48),new THREE.MeshStandardMaterial({color:col,emissive:col,emissiveIntensity:.4,transparent:true,opacity:.52,side:THREE.DoubleSide}),s*S*.38,S*1.22,0,0,0,s*.5);for(let f=0;f<3;f++)P(new THREE.PlaneGeometry(S*.15,S*.35),new THREE.MeshStandardMaterial({color:col,emissive:col,emissiveIntensity:.5,transparent:true,opacity:.42,side:THREE.DoubleSide}),s*(S*.28+f*S*.12),S*(.55-f*.1),S*.05,0,f*.25,s*.45);});
  [-1,1].forEach(s=>P(new THREE.SphereGeometry(S*.06,8,8),new THREE.MeshStandardMaterial({color:0,emissive:0x00ffff,emissiveIntensity:8,roughness:0}),s*S*.13,S*1.07,S*.22));
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.04,S*.06,S*.52,6),M(col,.3,dark),s*S*.22,S*.44,0,0,0,s*.55);P(new THREE.CylinderGeometry(S*.035,S*.045,S*.48,6),M(col,.3,dark),s*S*.38,S*.16,0,0,0,s*.75);});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.05,S*.04,S*.58,6),M(col,.25,dark),s*S*.14,S*(-.18),0);P(new THREE.SphereGeometry(S*.07,6,6),M(col,.3,dark),s*S*.14,S*(-.5),0);P(new THREE.CylinderGeometry(S*.04,S*.05,S*.42,6),M(col,.25,dark),s*S*.14,S*(-.7),0);P(new THREE.CylinderGeometry(S*.12,S*.1,S*.04,8),M(col,.2,dark),s*S*.14,S*(-.92),0);});
}

function _buildPyro(group,S,col,dark,genome,P,M){
  P(new THREE.CylinderGeometry(S*.25,S*.38,S*.88,10),M(col,.55,col,.3,.35),0,S*.28,-S*.1,.25);
  for(let i=0;i<4;i++)P(new THREE.CylinderGeometry(S*(.18-i*.02),S*(.22-i*.02),S*.22,8),M(col,.4,col,.3,.35),0,S*(.72+i*.18),S*(-i*.04));
  P(new THREE.BoxGeometry(S*.38,S*.28,S*.48),M(col,.5,col,.25,.4),0,S*1.14,S*.12);
  P(new THREE.BoxGeometry(S*.26,S*.18,S*.32),M(dark,.3,dark,.3,.4),0,S*1.08,S*.38);
  for(let i=0;i<5;i++){const a=(i/4-.5)*1.1;P(new THREE.ConeGeometry(S*.05,S*(.25+i*.04),6),M(0xff4400,3,0xff2200,.05,.6),Math.sin(a)*S*.18,S*(1.28+Math.cos(a)*.08),Math.cos(a)*S*.02,a*.5);}
  [-1,1].forEach(s=>P(new THREE.SphereGeometry(S*.065,8,8),new THREE.MeshStandardMaterial({color:0,emissive:0xff6600,emissiveIntensity:8,roughness:0}),s*S*.15,S*1.14,S*.36));
  for(let i=0;i<4;i++){const a=i/4*Math.PI*2;P(new THREE.CylinderGeometry(S*.06,S*.08,S*.04,8),new THREE.MeshStandardMaterial({color:0,emissive:0xff4400,emissiveIntensity:6,roughness:0}),Math.cos(a)*S*.3,S*.35,Math.sin(a)*S*.3-S*.1);}
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.04,S*.06,S*.85,6),M(dark,.2,dark,.4,.6),s*S*.55,S*.55,0,0,0,s*.7);for(let f=0;f<3;f++)P(new THREE.PlaneGeometry(S*(0.6-f*.1),S*(0.7-f*.08)),new THREE.MeshStandardMaterial({color:col,emissive:0xff2200,emissiveIntensity:.8+f*.4,transparent:true,opacity:.55-f*.1,side:THREE.DoubleSide}),s*(S*.42+f*S*.18),S*(.62-f*.12),0,0,0,s*(.55+f*.2));});
  [-1,1].forEach(s=>[-1,1].forEach(fwd=>{P(new THREE.CylinderGeometry(S*.1,S*.08,S*.38,8),M(col,.3,dark),s*S*.35,S*(-.12),fwd*S*.22,0,0,s*.3);P(new THREE.CylinderGeometry(S*.08,S*.1,S*.32,8),M(col,.3,dark),s*S*.42,S*(-.38),fwd*S*.25,0,0,s*.5);P(new THREE.BoxGeometry(S*.12,S*.05,S*.3),M(col,.2,dark),s*S*.44,S*(-.58),fwd*S*.27+S*.04);}));
  for(let i=0;i<9;i++)P(new THREE.SphereGeometry(S*(.2-i*.018),8,6),M(col,.35,col),0,S*(-.04-i*.12),S*(-0.08-i*.14));
  P(new THREE.ConeGeometry(S*.07,S*.32,6),M(0xff4400,3,0xff2200,.05,.6),0,S*(-.12),S*(-1.28),-.65);
}

function _buildGlacius(group,S,col,dark,genome,P,M){
  P(new THREE.BoxGeometry(S*.55,S*.78,S*.45),M(col,.35,col,.15,.7),0,S*.34,0);
  for(let i=0;i<4;i++){const offs=[-S*.28,S*.28,-S*.22+i*.05,S*.22-i*.05];P(new THREE.BoxGeometry(S*.18,S*.55,S*.07),new THREE.MeshStandardMaterial({color:0xaaddff,emissive:0x88ccff,emissiveIntensity:.5,roughness:.02,metalness:.8,transparent:true,opacity:.6}),offs[i%2]*1.05,S*.34,S*(.28-i*.04));}
  [-1,1].forEach(s=>{for(let i=0;i<4;i++)P(new THREE.ConeGeometry(S*.04,S*(.35+i*.06),6),new THREE.MeshStandardMaterial({color:0xcceeFF,emissive:0x88bbff,emissiveIntensity:1,roughness:.02,metalness:.9}),s*S*(.3+i*.06),S*(.82+i*.12),-S*.06,-.15,0,s*(.2+i*.12));});
  P(new THREE.BoxGeometry(S*.42,S*.32,S*.4),M(col,.4,col,.2,.65),0,S*.88,0);
  for(let i=0;i<6;i++){const a=(i/6-.5)*1.3;P(new THREE.ConeGeometry(S*.04,S*.32,6),new THREE.MeshStandardMaterial({color:0xddeeff,emissive:0x88bbff,emissiveIntensity:1.2,roughness:.02,metalness:.9}),Math.sin(a)*S*.18,S*1.08,0,a*.5);}
  P(new THREE.TorusGeometry(S*.1,S*.025,6,14),new THREE.MeshStandardMaterial({color:0,emissive:0x88eeff,emissiveIntensity:4,roughness:0}),0,S*.86,S*.22);
  [-1,1].forEach(s=>P(new THREE.SphereGeometry(S*.055,8,8),new THREE.MeshStandardMaterial({color:0,emissive:0x44ddff,emissiveIntensity:9,roughness:0}),s*S*.14,S*.89,S*.22));
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.12,S*.1,S*.45,8),M(col,.3,col,.2,.65),s*S*.42,S*.45,0,0,0,s*.4);P(new THREE.SphereGeometry(S*.13,8,8),M(col,.3,col,.2,.65),s*S*.56,S*.22,0);P(new THREE.BoxGeometry(S*.22,S*.2,S*.18),new THREE.MeshStandardMaterial({color:0xbbddff,emissive:0x88bbff,emissiveIntensity:.8,roughness:.02,metalness:.95,transparent:true,opacity:.75}),s*S*.7,S*(-.2),0);});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.14,S*.12,S*.5,8),M(col,.3,col,.2,.65),s*S*.22,S*(-.1),0);P(new THREE.SphereGeometry(S*.14,8,8),M(col,.3,col,.25,.65),s*S*.22,S*(-.38),0);P(new THREE.CylinderGeometry(S*.12,S*.14,S*.45,8),M(col,.3,col,.25,.65),s*S*.22,S*(-.65),0);P(new THREE.BoxGeometry(S*.28,S*.08,S*.4),M(col,.25,col,.2,.65),s*S*.2,S*(-.9),S*.05);P(new THREE.BoxGeometry(S*.14,S*.34,S*.08),new THREE.MeshStandardMaterial({color:0xbbddff,emissive:0x88bbff,emissiveIntensity:.6,roughness:.02,metalness:.95,transparent:true,opacity:.65}),s*S*.22,S*(-.6),S*.15);});
}

function _buildArachna(group,S,col,dark,genome,P,M){
  P(new THREE.SphereGeometry(S*.45,12,10),M(col,.5,col,.25,.4),0,S*.22,-S*.28);
  for(let i=0;i<4;i++)P(new THREE.TorusGeometry(S*(.1+i*.08),S*.01,4,20),M(0xdddddd,.5,0x888888,.4,.3),0,S*.22,-S*.28,Math.PI*.5);
  P(new THREE.SphereGeometry(S*.32,10,8),M(col,.45,dark,.3,.45),0,S*.38,S*.14);
  [[-0.2,.12],[.2,.12],[-.1,.22],[.1,.22],[-.22,.02],[.22,.02],[-.06,.05],[.06,.05]].forEach(([ex,ey])=>{const eye=new THREE.Mesh(new THREE.SphereGeometry(S*.038,6,6),new THREE.MeshStandardMaterial({color:0,emissive:0x22ff88,emissiveIntensity:6,roughness:0}));eye.position.set(ex*S,(0.38+ey)*S,S*.32);group.add(eye);});
  [-1,1].forEach(s=>{P(new THREE.CylinderGeometry(S*.05,S*.07,S*.22,6),M(dark,.2,dark,.4,.5),s*S*.1,S*.28,S*.42,.5);P(new THREE.ConeGeometry(S*.055,S*.22,6),M(0xaaaaaa,1.5,0x444444,.05,.95),s*S*.1,S*.15,S*.56,-.3);});
  for(let i=0;i<8;i++){const side=i<4?-1:1,li=i%4,a=(li/4-.5)*1.8+side*.05,lx=side*S*.32,lz=S*(-.08+li*.15);
    P(new THREE.CylinderGeometry(S*.04,S*.055,S*.45,6),M(col,.3,dark),lx+Math.cos(a)*S*.12,S*.28+li*.04,lz,0,0,side*(.8+li*.15));
    P(new THREE.SphereGeometry(S*.06,6,6),M(col,.35,dark),lx+side*S*(.4+li*.05),S*(.12-li*.06),lz);
    P(new THREE.CylinderGeometry(S*.03,S*.04,S*.52,6),M(col,.25,dark),lx+side*S*(.6+li*.06),S*(-.12-li*.06),lz,0,0,side*(1.2+li*.12));
    P(new THREE.ConeGeometry(S*.025,S*.12,5),M(0xcccccc,1.5,0x555555,.05,.95),lx+side*S*(.8+li*.07),S*(-.38-li*.04),lz,0,0,side*Math.PI*.5);}
  for(let i=0;i<3;i++)P(new THREE.CylinderGeometry(S*.04,S*.06,S*.12,6),M(col,.3,dark),0,S*(.14-i*.06),-S*.72+i*.02,.6);
}

function _buildVoid(group,S,col,dark,genome,P,M){
  P(new THREE.SphereGeometry(S*.45,16,14),new THREE.MeshStandardMaterial({color:0x000022,emissive:0x2200aa,emissiveIntensity:1.5,roughness:0,transparent:true,opacity:.55,side:THREE.DoubleSide}),0,S*.35,0);
  P(new THREE.SphereGeometry(S*.25,12,10),new THREE.MeshStandardMaterial({color:0,emissive:0x5500cc,emissiveIntensity:3,roughness:0,transparent:true,opacity:.8}),0,S*.35,0);
  P(new THREE.TorusGeometry(S*.38,S*.04,8,32),new THREE.MeshStandardMaterial({color:0,emissive:0x8800ff,emissiveIntensity:5,roughness:0}),0,S*.35,0);
  const orbCols=[0x8800ff,0x4400cc,0xaa44ff,0x660099];
  const frags=[];
  for(let i=0;i<12;i++){const orb=i/12*Math.PI*2,r=S*(.55+Math.random()*.3);
    const frag=new THREE.Mesh(new THREE.TetrahedronGeometry(S*(.04+Math.random()*.06)),new THREE.MeshStandardMaterial({color:0,emissive:orbCols[i%4],emissiveIntensity:4+Math.random()*3,roughness:0}));
    frag.position.set(Math.cos(orb)*r,S*.35+Math.sin(i*.8)*S*.18,Math.sin(orb)*r);
    frag.userData.orbitR=r;frag.userData.orbitA=orb;frag.userData.orbitS=.008+Math.random()*.012;frag.userData.orbitY=frag.position.y;
    group.add(frag);frags.push(frag);}
  group.userData.orbitFrags=frags;
  for(let t=0;t<6;t++){const ta=t/6*Math.PI*2,tl=S*(.4+Math.random()*.25);
    for(let s=0;s<5;s++){const st=s/4;P(new THREE.SphereGeometry(S*(.04-st*.025),5,5),new THREE.MeshStandardMaterial({color:0,emissive:0x6600cc,emissiveIntensity:2+st,roughness:0,transparent:true,opacity:.7-st*.1}),Math.cos(ta)*tl*st,S*.35+Math.sin(ta+st)*S*.2,Math.sin(ta)*tl*st);}}
  for(let i=0;i<3;i++){const a=i/3*Math.PI*2;P(new THREE.SphereGeometry(S*.04,6,6),new THREE.MeshStandardMaterial({color:0,emissive:0xffffff,emissiveIntensity:10,roughness:0}),Math.cos(a)*S*.28,S*.38,Math.sin(a)*S*.28);}
}
...
function startIdleAnim(group,archetype){
  const baseY=group.position.y,S=group.userData._S||1;let f=0;
  const frags=group.userData.orbitFrags;
  const anim=()=>{
    if(!group.parent)return;f++;
    group.position.y=baseY+Math.sin(f*.018)*.12;
    // Eye pulse
    group.children.forEach(c=>{if(c.isMesh&&c.material?.emissiveIntensity>3&&c.material?.color?.r<.05){if(!c._ei0)c._ei0=c.material.emissiveIntensity;c.material.emissiveIntensity=c._ei0+Math.sin(f*.14)*2;}});
    // Cosmic fragments orbit
    if(frags)frags.forEach(frag=>{frag.userData.orbitA+=frag.userData.orbitS;frag.position.x=Math.cos(frag.userData.orbitA)*frag.userData.orbitR;frag.position.z=Math.sin(frag.userData.orbitA)*frag.userData.orbitR;frag.position.y=frag.userData.orbitY+Math.sin(f*.025+frag.userData.orbitA)*S*.12;frag.rotation.x+=.04;frag.rotation.y+=.05;});
    // Fire pulse
    if(archetype.traits.fire)group.children.forEach(c=>{if(c.isMesh&&c.material?.emissive?.getHex()===0xff4400)c.material.emissiveIntensity=5+Math.sin(f*.22)*2.5;});
    // Ice shimmer
    if(archetype.traits.ice)group.children.forEach((c,idx)=>{if(c.isMesh&&c.material?.metalness>.8)c.material.emissiveIntensity=.5+Math.sin(f*.08+idx*.4)*.3;});
    // Sonic membrane ripple
    if(archetype.traits.sonic)group.children.forEach((c,idx)=>{if(c.geometry?.type==='PlaneGeometry'){c.scale.y=1+Math.sin(f*.12+idx)*.08;c.scale.x=1+Math.sin(f*.1+idx*.5)*.05;}});
...
function triggerSkillAnim(obj){
  ...
  const a=()=>{f++;if(f<8)obj.scale.setScalar(orig.x*(1+f*.025));else if(f<16)obj.scale.setScalar(orig.x*(1.18-(f-8)*.025));else{obj.scale.copy(orig);markDirty(2);return;}markDirty(2);requestAnimationFrame(a);};
  requestAnimationFrame(a);
  const arch=obj.userData.archetype;
  screenFlash(arch?.traits?.fire?'rgba(255,100,0,.5)':arch?.traits?.ice?'rgba(100,200,255,.35)':'rgba(150,120,255,.35)',100);
}
...
async function testAndSaveKey(key){
  ...
  if(st){st.className='nh-key-status';st.textContent='Testando conexão…';}
  try{
    const r=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
    ...
    const d=await r.json();
    if(d.error){if(st){st.className='nh-key-status err';st.textContent='Erro: '+d.error.message;}return;}
    CLAUDE_API_KEY=key;localStorage.setItem('nh_claude_key',key);
    if(st){st.className='nh-key-status ok';st.textContent='✔ Chave válida! IA ativada.';}
    const btn=$('nh-key-btn');if(btn)btn.classList.add('active');
    setTimeout(()=>$('nh-key-modal')?.classList.add('hidden'),1200);
    toast('IA ativada com sucesso!',2500);
  }catch{if(st){st.className='nh-key-status err';st.textContent='Erro de conexão. Verifique a chave.';}}
}
...
function init(){
  ...
  // Hook active object changed → show skills toggle button (not auto-open panel)
