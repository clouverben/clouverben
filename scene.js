...
export const app = {
  ...
  _userAxesVisible: true,   // Settings menu toggle — persists across per-frame helper resets
  floor: null,
...
export function initScene(container = document.body) {
  ...
  scene.background = new THREE.Color(0x373737);   // default viewport gray
  ...
  // Three.js names the gizmo's internal handle meshes "X", "Y", "Z", "XY", "E", etc.
  // Without this, those names leak into object lists (attach picker, hierarchy,
  // object counters) even though the gizmo itself is invisible in the 3D view.
  tcHelper.traverse(c => { c.userData.isHelper = true; });
  scene.add(tcHelper);
