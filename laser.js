...
class LaserParticleSystem extends THREE.Object3D {
    ...
    _build() {
        ...
        // ── 2. GLOW em 2 camadas (cilindro grande r:1.00 removido) ───────────
        this._glowMats = [];
        [
            { r: 0.10, op: 0.95 },
            { r: 0.22, op: 0.65 },
        ...
        for (let i = 0; i < 12; i++) {
            ...
            arc.material.color.set(i % 3 === 0
                ...
                : new THREE.Color(0xffee44));          // amarelo
            this._arcs.push({ mesh: arc, ttl: Math.random() * 0.08 });
            this.add(arc);
        }

        // ── 4. BIFURCAÇÕES (8 arcos finos que partem do meio) ────────────────
        for (let i = 0; i < 8; i++) {
    ...
    // Arco primário — ziguezague ao longo do eixo Y inteiro
    ...
    // Bifurcação — trecho curto que parte de um ponto aleatório do feixe
    _buildFork() {
        const H      = this.beamHeight;
        const segs   = 8 + Math.floor(Math.random() * 6);
        const startY = Math.random() * H;
        const len    = 0.8 + Math.random() * 1.8;   // comprimento da bifurcação
    ...
    update() {
        ...
        this._arcs.forEach(obj => {
            ...
            // flicker por arco — efeito raio real
            obj.mesh.material.opacity = (Math.random() > 0.15)
                ? (0.45 + Math.random() * 0.55) * this._opacity
                : 0;  // apaga completamente às vezes = mais realista
        });
