...
class SaveSystem {
    ...
    _triggerDownload(bytes, filename) {
        // Blob URL + setAttribute: único método confiável para arquivos grandes.
        // Data URI quebra no Chrome para arquivos > 2 MB (modelos 3D embutidos).
        // setAttribute em vez de .download = garante compatibilidade máxima.
        // revokeObjectURL após 60s — revogar antes cancela o download em progresso.
    ...
    // ── Extrai textura para DataURL ─────────────────────────────────────
    _textureToDataURL(texture, maxSize = 2048) {
        ...
        try {
            ...
            // Mantém proporção e respeita maxSize
            ...
            // Tenta PNG primeiro (sem perda), cai pra JPEG se canvas estiver tainted
            try { return canvas.toDataURL('image/png'); }
            catch (_) {}
            try { return canvas.toDataURL('image/jpeg', 0.92); }
            catch (secErr) {
                // Canvas tainted (imagem cross-origin) — tenta pegar pixels via ImageBitmap
                console.warn('[SaveSystem] Canvas tainted, tentando ImageBitmap:', secErr.message);
            }

            // Fallback: se a textura tem _src original gravado, usa direto
            if (texture.userData?._srcDataURL) return texture.userData._srcDataURL;
            return null;
        } catch (e) {
    ...
    // Problema: applyLODToModel() substitui meshes por THREE.LOD na cena.
    // THREE.LOD.toJSON() não serializa os níveis corretamente → modelo perdido.
    // Solução: constrói um Group temporário com apenas o nível 0 (malha original)
    // de cada LOD, serializando a geometria intacta.
    _serializeModel(obj) {
        // 1. Coleta TODAS as texturas do objeto (incluindo dentro de LODs)
        const texByUUID = new Map();
        obj.traverse(child => {
            const mesh = child.isLOD ? child.levels?.[0]?.object : (child.isMesh ? child : null);
        ...
        // 2. Constrói Group temporário substituindo LODs pela malha do nível 0
        //    e clonando SkinnedMeshes sem o skeleton (que não serializa bem).
        function _flattenForJSON(src) {
            if (src.isLOD) {
                // Pega só o nível 0 (geometria original completa)
                const lvl0 = src.levels?.[0]?.object;
                if (!lvl0) return null;
                const m = lvl0.clone(false);
                m.name = src.name || lvl0.name;
            ...
            if (src.isSkinnedMesh) {
                // SkinnedMesh com skeleton quebra o toJSON — serializa como Mesh simples
            ...
            if (src.isMesh) return src.clone(false);
            // Group / Object3D — recria e processa filhos recursivamente
            ...
            src.children.forEach(c => {
                const flat = _flattenForJSON(c);
                if (flat) g.add(flat);
            });
            return g;
        }

        const serTarget = _flattenForJSON(obj) || obj;

        let json;
        try {
            json = serTarget.toJSON();
        } catch (e) {
            console.warn('[SaveSystem] toJSON falhou:', e.message);
            return null;
        }

        // 3. Corrige URLs de imagem — blob: e http: precisam virar dataURL embutida
    ...
    export() {
        ...
        setTimeout(async () => {
            try {
                ...
                this._triggerDownload(nexBytes, 'project.nex');

                this._toast(`📦 Exportado! (${sizeKB} KB) — project.nex`);
    ...
    // ── Importar arquivo .nex ────────────────────────────────────────────
    async importFromFile(file) {
        const name = file.name.toLowerCase();

        // Aceita SOMENTE .nex — nenhum outro formato
