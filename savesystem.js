...
class SaveSystem {
    ...
    _triggerDownload(bytes, filename) {
        // showSaveFilePicker abre o dialogo nativo do SO — unico jeito 100% garantido
        // de salvar como project.nex sem o browser renomear pra download.bin.
        // Suportado em Chrome e Edge. Firefox cai no fallback abaixo.
        if (typeof window.showSaveFilePicker === 'function') {
            window.showSaveFilePicker({
                suggestedName: filename,
                types: [{ description: 'Nexus Project (.nex)', accept: { 'application/octet-stream': ['.nex'] } }],
            }).then(handle => handle.createWritable())
              .then(writable => writable.write(bytes).then(() => writable.close()))
              .catch(e => { if (e.name !== 'AbortError') console.error('[SaveSystem] Erro ao salvar:', e); });
            return;
        }
        // Fallback Firefox/Safari: Blob URL com cleanup tardio
