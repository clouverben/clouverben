...
class SaveSystem {
    ...
    async _triggerDownload(bytes, filename) {
        const blob = new Blob([bytes], { type: 'application/octet-stream' });

        // showSaveFilePicker: abre diálogo nativo "Salvar como" com nome já correto
        if (typeof window.showSaveFilePicker === 'function') {
            try {
                const handle = await window.showSaveFilePicker({
                    suggestedName: filename,
                    types: [{ description: 'Nexus Engine Project', accept: { 'application/octet-stream': ['.nex'] } }],
                });
        ...
        // Fallback: blob: URL — o browser respeita o atributo download em blob same-origin
    ...
    export() {
        ...
        setTimeout(async () => {
            try {
                ...
                await this._triggerDownload(nexBytes, 'project.nex');
