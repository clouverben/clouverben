    ...
    container.innerHTML = `
        ...
        <div id="dopesheet-panel" class="tl-tool-panel hidden">
            <div class="tl-tool-header">
                ...
                <button class="tl-tool-close" id="dopesheet-close">✕</button>
            </div>
        ...
        <div id="graph-panel" class="tl-tool-panel hidden">
            <div class="tl-tool-header">
                ...
                <button class="tl-tool-close" id="graph-close">✕</button>
            </div>
        ...
        <div id="onion-panel" class="tl-tool-panel tl-panel-small hidden">
            <div class="tl-tool-header">
                ...
                Onion Skin
                <button class="tl-tool-close" id="onion-close">✕</button>
            </div>
        ...
        <div id="marker-panel" class="tl-tool-panel hidden">
            <div class="tl-tool-header">
                ...
                <button class="tl-tool-close" id="marker-close">✕</button>
            </div>
            <div class="marker-body">
                <div class="marker-add-row">
                    ...
                    <button id="marker-add-btn" class="marker-add-btn">＋</button>
                </div>
        ...
        <div id="loop-panel" class="tl-tool-panel tl-panel-small hidden">
            <div class="tl-tool-header">
                ...
                Loop Region
                <button class="tl-tool-close" id="loop-close">✕</button>
            </div>
            <div class="loop-body">
                ...
                <div class="loop-row"><label class="loop-label" style="color:#6ec6ff">In</label><input type="number" id="loop-in" class="loop-num-input" value="0" min="0"><button id="loop-in-set" class="loop-set-btn">◀ Atual</button></div>
                <div class="loop-row"><label class="loop-label" style="color:#ffb347">Out</label><input type="number" id="loop-out" class="loop-num-input" value="100" min="0"><button id="loop-out-set" class="loop-set-btn">Atual ▶</button></div>
            </div>
        </div>
        ...
        <div id="autokey-panel" class="tl-tool-panel tl-panel-small hidden">
            <div class="tl-tool-header">
                ...
                Auto-Key
                <button class="tl-tool-close" id="autokey-close">✕</button>
            </div>
        ...
        <div class="timeline-bar">
            ...
            <div class="tl-clock-group">
                <button id="tl-fps-btn" class="tl-gear-btn" title="Configurações de FPS">⚙</button>
            ...
            <div class="tl-right-tools">
                <button id="tl-dopesheet-btn" class="tl-right-btn" title="Dope Sheet (D)">
                ...
                <button id="tl-graph-btn" class="tl-right-btn" title="Graph Editor (G)">
                    <svg viewBox="0 0 16 16" width="15" height="15"><polyline points="1,14 4,8 8,11 12,4 15,2" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round"/></svg>
                </button>
                <button id="tl-onion-btn" class="tl-right-btn" title="Onion Skin (O)">
                    <svg viewBox="0 0 16 16" width="15" height="15"><circle cx="8" cy="8" r="5.5" stroke="currentColor" stroke-width="1.6" fill="none"/><circle cx="8" cy="8" r="3" stroke="currentColor" stroke-width="1.2" fill="none" opacity=".55"/></svg>
                </button>
                <div class="tl-right-sep"></div>
                <button id="tl-marker-btn" class="tl-right-btn" title="Marcadores (M)">
                    <svg viewBox="0 0 16 16" width="15" height="15"><path d="M4 2h8v9l-4 3-4-3z" stroke="currentColor" stroke-width="1.5" fill="none"/></svg>
                </button>
                <button id="tl-loop-btn" class="tl-right-btn" title="Loop Region (L)">
                    <svg viewBox="0 0 16 16" width="15" height="15"><path d="M3 8a5 5 0 1 1 2 4" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round"/><polyline points="3,4 3,8 7,8" stroke="currentColor" stroke-width="1.5" fill="none"/></svg>
                </button>
                <button id="tl-autokey-btn" class="tl-right-btn" title="Auto-Key (A)">
                    <svg viewBox="0 0 16 16" width="15" height="15"><rect x="1" y="5" width="9" height="6" rx="1" stroke="currentColor" stroke-width="1.5" fill="none"/><circle cx="13" cy="7" r="2.2" stroke="currentColor" stroke-width="1.4" fill="none"/><line x1="13" y1="9.2" x2="13" y2="12" stroke="currentColor" stroke-width="1.5"/></svg>
                </button>
            </div>
        </div>
    `;
...
#timeline-container {
    ...
    left: 0;
    right: 0;
    height: var(--bar-h-bot, 54px);   /* mesma altura e largura que #bottomBar */
    z-index: 200;
...
#timeline-container .timeline-bar {
    ...
    gap: 6px;
    padding: 0 10px;
    height: 100%;                    /* fills the 54px container exactly */
}
.tl-btn {
    flex-shrink: 0;
...
.tl-kf-btn { font-size: 12px; color: #ffd95c; border-color: rgba(255,217,92,.3); }
.tl-kf-btn:hover { background: rgba(255,217,92,.12); border-color: rgba(255,217,92,.5); }
...
.timeline-ruler { position: absolute; top: 0; left: 0; height: 14px; pointer-events: none; }
.ruler-tick { position: absolute; top: 0; bottom: 0; width: 1px; background: rgba(255,255,255,.08); }
.ruler-tick-major { background: rgba(255,255,255,.18); }
.ruler-label { position: absolute; top: 1px; left: 3px; font-size: 8px; color: rgba(255,255,255,.3); font-family: monospace; white-space: nowrap; }
.timeline-playhead {
...
.tl-clock-group { display: flex; align-items: center; gap: 4px; flex-shrink: 0; }
.tl-gear-btn {
...
.tl-gear-btn:hover { background: rgba(255,255,255,.1); color: #fff; }
.tl-clock { display: flex; align-items: baseline; gap: 2px; }
.tl-clock-display, #tl-clock-display { font-size: 16px; font-weight: 700; font-family: 'Courier New', monospace; color: rgba(255,255,255,.7); min-width: 40px; }
.tl-clock-label { font-size: 10px; color: rgba(255,255,255,.3); }

.tl-right-tools { display: flex; align-items: center; gap: 3px; flex-shrink: 0; }
.tl-right-btn {
...
function updatePlayhead() {
    ...
    const clockEl  = document.getElementById('tl-clock-display');
    ...
    if (clockEl) clockEl.textContent = String(AnimState.currentFrame).padStart(4, '0');
...
function renderGraphEditor() {
    ...
    const au=getActiveObject()?.uuid, uuids=au&&AnimState.keyframes[au]?[au]:Object.keys(AnimState.keyframes);
...
function updateOnionGhosts() {
    ...
    const obj=getActiveObject(); if(!obj||!AnimState.keyframes[obj.uuid])return;
...
function renderMarkerList() {
    ...
    list.innerHTML='';
    keys.forEach(frame=>{const item=document.createElement('div');item.className='marker-item';item.innerHTML=`<div class="marker-color"></div><span class="marker-frame">${frame}</span><span class="marker-name">${AnimState.markers[frame]}</span><button class="marker-del">✕</button>`;item.querySelector('.marker-name').addEventListener('click',()=>seekFrame(frame));item.querySelector('.marker-frame').addEventListener('click',()=>seekFrame(frame));item.querySelector('.marker-del').addEventListener('click',e=>{e.stopPropagation();deleteMarker(frame);});list.appendChild(item);});
}
...
export const AnimationSystem = {
    toggle() {
        ...
        c.style.display = AnimState.visible ? 'block' : 'none';
