...
// THREE and mergeGeometries are exposed as globals by main.js (window.THREE, window._mergeGeometries)

window.addEventListener('load', () => setTimeout(initNexusHelper, 400));
...
function getSelected() {
  ...
  if (tc?.object) return tc.object;
  const scene = window._nexusScene;
  if (!scene) return null;
  let found = null;
  scene.traverse(o => { if (o.userData?.selected) found = o; });
  return found;
}
...
function getSelectedMeshes() {
  ...
  const scene = window._nexusScene;
  const list = [];
  if (scene) scene.traverse(o => { if (o.isMesh && o.userData?.selected) list.push(o); });
  if (list.length === 0 && tc?.object?.isMesh) list.push(tc.object);
  return list;
}
...
function transformMech() {
  ...
  // Convert all fin positions to local space (mesh.parent = scene, so subtract mesh.position)
  // Parts are children of mesh so they move/rotate with it
  const toLocal = (worldPos) => {
  ...
  const parts = attachments.map(a => {
    ...
    // Start at mesh local origin (center of object) at scale 0
    ...
    mesh.add(m);  // child of mesh — moves with it
    const finLocal = toLocal(a.fin);
    return { mesh:m, fin:finLocal, initRot:a.rot||new THREE.Euler(), label:a.label };
  });
...
function startMechIdle(parts, center, size, baseMesh) {
  // Parts are children of baseMesh — animate in local space only
  const legL = parts.find(p=>p.label==='lL');
  const legR = parts.find(p=>p.label==='lR');
  let t = 0;
  function tick() {
    if (!baseMesh.parent) return; // mesh removed from scene
    t += 0.02;
    parts.forEach(p => {
      ...
      if (p.label==='sL') p.mesh.rotation.z = Math.sin(t*.9)*0.05;
      if (p.label==='sR') p.mesh.rotation.z = -Math.sin(t*.9)*0.05;
    });
...
async function runMerge(mode) {
  ...
  // ── DNA ────────────────────────────────────────────────────────
    ...
    const tgt = new THREE.Vector3().addVectors(m1.position,m2.position).multiplyScalar(0.5);
    const p1s=m1.position.clone(), p2s=m2.position.clone();
    const startT = performance.now();
    await new Promise(res => {
      function f(now) {
        const t=Math.min((now-startT)/800,1);
        m1.position.lerpVectors(p1s,tgt,t);
        m2.position.lerpVectors(p2s,tgt,t);
    ...
    [m1,m2].forEach(m => {
      if (m.material?.color) m.material.color.copy(blended);
      if ('metalness' in (m.material??{})) m.material.metalness=avgM;
      if ('roughness' in (m.material??{})) m.material.roughness=avgR;
      if ('emissive'  in (m.material??{})) m.material.emissive.copy(blended).multiplyScalar(0.12);
      m.material.needsUpdate=true;
    });
    spawnBurst(m1, blended);
    showToast('✦ Fusão DNA completa!','success');
    const {shape:s1,mineral:min1}=identifyShape(m1), {shape:s2,mineral:min2}=identifyShape(m2);
    resultTitle.textContent='⟡ Fusão DNA';
    resultText.textContent=`${s1} (${min1}) ✕ ${s2} (${min2}) — cor, metalness e roughness misturados. Objetos convergidos.`;
    resultEl.classList.add('visible');
...
f (window._nexusTransformControls) window._nexusTransformControls.attach(hybrid);
