...
// ================================================================
//  VOLUMETRIC FIRE & SMOKE SIMULATION
//  GPU-only: ShaderMaterial animado, zero CPU por frame
// ================================================================
class FireSmokeSimulation {
  constructor(scene){this.scene=scene;this.active=new Map();}

  _fireMat(color,mode,intensity,height,turb){
    const c=new THREE.Color(color);
    return new THREE.ShaderMaterial({
      uniforms:{time:{value:0},color:{value:c},intensity:{value:intensity},height:{value:height},turb:{value:turb},mode:{value:mode==='smoke'?1.0:mode==='both'?2.0:0.0}},
      ...
      fragmentShader:`
        uniform float time,intensity,height,turb,mode;uniform vec3 color;varying vec3 vPos;varying vec2 vUv;
        float h(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5);}
        float n(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(h(i),h(i+vec2(1,0)),f.x),mix(h(i+vec2(0,1)),h(i+vec2(1,1)),f.x),f.y);}
        float fbm(vec2 p){return n(p)*.5+n(p*2.1+time*.3)*.25+n(p*4.3-time*.15)*.125;}
        void main(){
          float t=time;
          float y=(vPos.y/height);if(y<0.||y>1.){discard;}
          float r=length(vPos.xz)/(height*0.5);if(r>1.0)discard;
          // Fire column shape
          float cone=clamp(1.-r*1.1,0.,1.);
          cone=pow(cone,1.5+y*2.);
          // Noise rising
          vec2 np=vec2(vPos.x*turb,vPos.z*turb)*1.8+vec2(t*.0,t*.4);
          float noise=fbm(np);
          float rising=fbm(vec2(vPos.x*turb+t*.05,vPos.z*turb)*2.+vec2(0.,t*.6));
          // Fire color gradient: white->yellow->orange->red->black
          vec3 fireCol;float fl=y+noise*.3;
          if(fl<.15)fireCol=vec3(1.,1.,.85);
          else if(fl<.4)fireCol=mix(vec3(1.,1.,.7),vec3(1.,.6,.1),(fl-.15)/.25);
          else if(fl<.75)fireCol=mix(vec3(1.,.6,.1),color,(fl-.4)/.35);
          else fireCol=mix(color,vec3(.05,.02,.01),(fl-.75)/.25);
          // Smoke: dark grey billowing
          vec3 smokeCol=vec3(.25,.22,.2)*(1.-rising*.4);
          float smokeA=clamp(rising*1.2*(1.-y*.5)*cone*.8,0.,1.)*intensity;
          float fireA=clamp(noise*1.5*(1.-y*.8)*cone,0.,1.)*intensity;
          // Mode blend
          vec3 col; float alpha;
          if(mode<.5){col=fireCol;alpha=fireA;}
          else if(mode<1.5){col=smokeCol;alpha=smokeA*.7;}
          else{// both: fire at base, smoke at top
            float blend=smoothstep(.3,.8,y);
            col=mix(fireCol,smokeCol,blend);alpha=mix(fireA,smokeA,blend);}
  ...
  apply(obj,config){
    if(this.active.has(obj.uuid))this.remove(obj);
    const box=new THREE.Box3().setFromObject(obj),size=new THREE.Vector3();box.getSize(size);
    const h=config.height||2.0,r=(size.x+size.z)*.35;
    // 3 overlapping cylinders for volume
    const group=new THREE.Group();
    const center=new THREE.Vector3();box.getCenter(center);
    group.position.copy(new THREE.Vector3(center.x,box.min.y,center.z));
    const mats=[];
    for(let i=0;i<3;i++){
      const geo=new THREE.CylinderGeometry(r*(1+i*.15),r*.6,h,16,12,true);
      const mat=this._fireMat(config.color||'#ff4400',config.mode||'fire',config.intensity||1.0,h,config.turb||1.0);
      mat.uniforms.time.value=i*1.3;
      const mesh=new THREE.Mesh(geo,mat);
      mesh.position.y=h*.5;
      mesh.rotation.y=i*Math.PI*.667;
      mesh.layers.enable(1);
      group.add(mesh);mats.push(mat);
    }
    // Point light for fire glow
    const col=new THREE.Color(config.color||'#ff4400');
    const light=new THREE.PointLight(col.getHex(),(config.intensity||1)*1.5,h*2.5);
    light.position.y=h*.3;group.add(light);
    this.scene.add(group);
    this.active.set(obj.uuid,{group,mats,light,config});
    window._nexusInvalidateBloom?.();window._nexusMarkDirty?.(4);
  }

  update(dt){
    this.active.forEach(d=>{
      d.mats.forEach((m,i)=>{m.uniforms.time.value+=dt*(d.config.turb||1)*(.8+i*.15);});
      // Flicker light
      if(d.light){const f=.8+Math.sin(Date.now()*.012)*Math.random()*.4;d.light.intensity=(d.config.intensity||1)*1.5*f;}
      window._nexusMarkDirty?.(1);
    });
  }

  remove(obj){
    const d=this.active.get(obj.uuid);if(!d)return;
    this.scene.remove(d.group);d.group.traverse(c=>{c.geometry?.dispose();c.material?.dispose?.();});
    this.active.delete(obj.uuid);window._nexusInvalidateBloom?.();
  }
}

// ================================================================
//  WATER / LIQUID SIMULATION
//  Plane com vertex shader de ondas + ShaderMaterial caustiques
// ================================================================
class WaterSimulation {
  constructor(scene){this.scene=scene;this.active=new Map();}

  _waterMat(color,type,waveH,reflect){
    const c=new THREE.Color(color);
    const presets={water:{deep:new THREE.Color('#0369a1'),foam:new THREE.Color('#e0f2fe')},lava:{deep:new THREE.Color('#7f1d1d'),foam:new THREE.Color('#f97316')},acid:{deep:new THREE.Color('#14532d'),foam:new THREE.Color('#a3e635')}};
    const p=presets[type]||presets.water;
    return new THREE.ShaderMaterial({
      uniforms:{time:{value:0},color:{value:c},deepColor:{value:p.deep},foamColor:{value:p.foam},waveH:{value:waveH},speed:{value:1.0},reflectivity:{value:reflect}},
      vertexShader:`
        uniform float time,waveH,speed;varying vec3 vNorm;varying vec2 vUv;varying float vHeight;
        float wave(vec2 p,float freq,float phase){return sin(p.x*freq+time*speed+phase)*cos(p.y*freq*.7+time*speed*.8+phase);}
        void main(){
          vUv=uv;vec3 p=position;
          float w=wave(p.xz,2.1,0.)+wave(p.xz,3.7,1.3)*.5+wave(p.xz,6.1,.8)*.25;
          p.y+=w*waveH;vHeight=w*.5+.5;
          // Normal estimation
          float eps=.05;float wx=wave(p.xz+vec2(eps,0.),2.1,0.)+wave(p.xz+vec2(eps,0.),3.7,1.3)*.5;float wz=wave(p.xz+vec2(0.,eps),2.1,0.)+wave(p.xz+vec2(0.,eps),3.7,1.3)*.5;
          vNorm=normalize(vec3(-(wx-w)/eps*waveH,1.,(-(wz-w)/eps*waveH)));
          gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.);}`,
      fragmentShader:`
        uniform vec3 color,deepColor,foamColor;uniform float time,reflectivity,waveH;varying vec3 vNorm;varying vec2 vUv;varying float vHeight;
        float h(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5);}
        float caust(vec2 uv){vec2 i=floor(uv*8.);float n=h(i)*.5+h(i+vec2(1,0))*.3+h(i+vec2(0,1))*.2;return n;}
        void main(){
          vec3 col=mix(deepColor,color,vHeight);
          float foam=smoothstep(.6,1.,vHeight);
          // Caustics
          float c=caust(vUv*3.+time*.1)*caust(vUv*5.-time*.07);
          col+=foamColor*c*.35;
          col=mix(col,foamColor,foam*.4);
          // Fresnel
          float fres=pow(1.-abs(dot(vNorm,vec3(0,1,0))),3.)*reflectivity;
          col=mix(col,vec3(.9,.95,1.),fres*.5);
  ...
  apply(obj,config){
    ...
    const box=new THREE.Box3().setFromObject(obj),size=new THREE.Vector3();box.getSize(size);
    const center=new THREE.Vector3();box.getCenter(center);
    const W=Math.max(size.x,size.z),D=Math.max(size.x,size.z);
    const geo=new THREE.PlaneGeometry(W,D,32,32);geo.rotateX(-Math.PI/2);
    const mat=this._waterMat(config.color||'#0ea5e9',config.type||'water',config.waveH||.18,config.reflect||.7);
    mat.uniforms.speed.value=config.speed||1.0;
    const mesh=new THREE.Mesh(geo,mat);mesh.position.copy(center);mesh.position.y=box.max.y+.02;
    mesh.castShadow=false;mesh.receiveShadow=true;mesh.layers.enable(1);
    this.scene.add(mesh);this.active.set(obj.uuid,{mesh,mat,config});
    window._nexusMarkDirty?.(4);
  }

  update(dt){
    this.active.forEach(d=>{d.mat.uniforms.time.value+=dt;window._nexusMarkDirty?.(1);});
  }

  remove(obj){
    const d=this.active.get(obj.uuid);if(!d)return;
    this.scene.remove(d.mesh);d.mesh.geometry.dispose();d.mat.dispose();this.active.delete(obj.uuid);
  }
}

// ================================================================
//  SAND / GRANULAR SIMULATION
//  InstancedMesh leve, atualizado a cada 2 frames
// ================================================================
class SandSimulation {
  constructor(scene){this.scene=scene;this.active=new Map();this._frame=0;}

  apply(obj,config){
    if(this.active.has(obj.uuid))this.remove(obj);
    const box=new THREE.Box3().setFromObject(obj),size=new THREE.Vector3();box.getSize(size);
    const center=new THREE.Vector3();box.getCenter(center);
    const count=parseInt(config.count)||300;
    const sz=config.size||.04;const color=new THREE.Color(config.color||'#d4a55a');
    const geo=new THREE.SphereGeometry(sz,4,3); // low-poly grain
    ...
    // Init particles above object
    const particles=[];const dummy=new THREE.Object3D();
    const rng=s=>Math.abs(Math.sin(s*127.1+311.7)*43758.5453%1);
    const floor=box.min.y-sz;
    for(let i=0;i<count;i++){
      const px=center.x+(rng(i*3.1)-.5)*size.x*.9;
      const py=box.max.y+(rng(i*5.3))*size.y*2;
      const pz=center.z+(rng(i*7.7)-.5)*size.z*.9;
      particles.push({p:new THREE.Vector3(px,py,pz),v:new THREE.Vector3((rng(i*11.1)-.5)*.02,-(rng(i*13.3))*.05,(rng(i*17.7)-.5)*.02),r:new THREE.Euler(rng(i)*Math.PI,rng(i*2)*Math.PI,0)});
      dummy.position.set(px,py,pz);dummy.rotation.copy(particles[i].r);dummy.updateMatrix();mesh.setMatrixAt(i,dummy.matrix);
    }
    this.scene.add(mesh);this.active.set(obj.uuid,{mesh,particles,config:{...config,floor,center,size},dummy});
    window._nexusMarkDirty?.(4);
  }

  update(dt){
    this._frame++;if(this._frame%2!==0)return; // throttle to 30fps
    const g=dt*2; // gravity step
    this.active.forEach(d=>{
      const{particles,mesh,config,dummy}=d;const grav=config.gravity||9.8;const floor=config.floor;
      let dirty=false;
      for(let i=0;i<particles.length;i++){
        const p=particles[i];if(p.p.y<=floor+.001&&Math.abs(p.v.y)<.001)continue;// settled
        p.v.y-=grav*g*g;p.v.x*=.995;p.v.z*=.995;
        p.p.addScaledVector(p.v,1);
        if(p.p.y<floor){p.p.y=floor;p.v.y*=-.25;p.v.x+=( Math.random()-.5)*.008;p.v.z+=(Math.random()-.5)*.008;}
        dummy.position.copy(p.p);dummy.rotation.copy(p.r);dummy.updateMatrix();mesh.setMatrixAt(i,dummy.matrix);dirty=true;
      }
      if(dirty){mesh.instanceMatrix.needsUpdate=true;window._nexusMarkDirty?.(1);}
    });
  }

  remove(obj){
    const d=this.active.get(obj.uuid);if(!d)return;
    this.scene.remove(d.mesh);d.mesh.geometry.dispose();d.mesh.material.dispose();this.active.delete(obj.uuid);
  }
}

// ================================================================
//  JELLY / ELASTIC SIMULATION
//  Vertex displacement por spring mass — qualidade gosma, leve
// ================================================================
class JellySimulation {
  ...
  apply(obj,config){
    if(this.active.has(obj.uuid))this.remove(obj);
    const box=new THREE.Box3().setFromObject(obj),size=new THREE.Vector3();box.getSize(size);
    const color=new THREE.Color(config.color||'#f472b6');
    const s=Math.max(size.x,size.y,size.z);

    // Clone geometry for jelly body
    const shape=obj.userData?.shapeType||'box';
    let geo;
    if(shape==='sphere')geo=new THREE.SphereGeometry(s*.52,32,24);
    else if(shape==='cylinder')geo=new THREE.CylinderGeometry(size.x*.5,size.x*.5,size.y,24,8);
    else if(shape==='torus')geo=new THREE.TorusGeometry(size.x*.35,size.x*.13,16,48);
    else geo=new THREE.BoxGeometry(size.x,size.y,size.z,8,8,8);

    // Store rest positions
    const restPos=new Float32Array(geo.attributes.position.array);
    const velArr=new Float32Array(restPos.length); // velocity per vertex

    const mat=new THREE.MeshPhysicalMaterial({
      color,emissive:color.clone().multiplyScalar(.15),roughness:.05,metalness:0,
      transmission:.4,thickness:s*.3,ior:1.35,clearcoat:1.,clearcoatRoughness:.02,
      transparent:true,opacity:config.opacity||.75,side:THREE.FrontSide
    });
    // Outer shell
    const shellMat=new THREE.MeshPhysicalMaterial({color:color.clone().multiplyScalar(.7),roughness:0,metalness:0,transparent:true,opacity:.25,side:THREE.BackSide,blending:THREE.AdditiveBlending,depthWrite:false});

    const mesh=new THREE.Mesh(geo,mat);mesh.castShadow=true;mesh.layers.enable(1);
    const shell=new THREE.Mesh(geo.clone(),shellMat);shell.scale.setScalar(1.05);

    const group=new THREE.Group();
    group.position.copy(obj.position);group.rotation.copy(obj.rotation);group.scale.copy(obj.scale);
    group.add(mesh);group.add(shell);

    // Interior glow
    const glowGeo=new THREE.SphereGeometry(s*.15,8,6);
    const glowMat=new THREE.MeshBasicMaterial({color:color.clone().multiplyScalar(1.5),transparent:true,opacity:.6,blending:THREE.AdditiveBlending,depthWrite:false});
    const glow=new THREE.Mesh(glowGeo,glowMat);glow.layers.enable(1);group.add(glow);

    obj.visible=false;
    this.scene.add(group);
    this.active.set(obj.uuid,{group,mesh,shell,glow,geo,restPos,velArr,config,obj,time:0,squashV:new THREE.Vector3(1,1,1),squashVel:new THREE.Vector3()});
    window._nexusInvalidateBloom?.();window._nexusMarkDirty?.(4);
  }

  _impulse(inst,strength){
    // Apply random impulse to vertex velocities
    const v=inst.velArr;const rng=()=>Math.random()-.5;
    for(let i=0;i<v.length;i+=3){v[i]+=rng()*strength;v[i+1]+=rng()*strength;v[i+2]+=rng()*strength;}
    // Also squash the whole body
    inst.squashVel.set((Math.random()-.5)*.3,(Math.random()-.5)*.3,(Math.random()-.5)*.3).multiplyScalar(strength*5);
  }

  update(dt){
    this._frame++;const throttle=this._frame%2===0; // 30fps cap on vertex sim
    this.active.forEach(inst=>{
      inst.time+=dt;const t=inst.time;
      const cfg=inst.config;
      const elastic=cfg.elastic||.4,damp=cfg.damp||.92,freq=cfg.freq||2.0;

      // Whole-body squash & stretch spring (runs every frame, cheap)
      const target=new THREE.Vector3(1,1,1);
      inst.squashVel.addScaledVector(target.sub(inst.squashV),elastic*.15);
      inst.squashVel.multiplyScalar(damp);
      inst.squashV.add(inst.squashVel);
      // Gentle idle wobble
      inst.squashV.x=1+Math.sin(t*freq+0)*(.04+(1-damp)*.1);
      inst.squashV.y=1+Math.sin(t*freq*1.3+1)*(.04+(1-damp)*.1);
      inst.squashV.z=1+Math.cos(t*freq*.9+2)*(.04+(1-damp)*.1);
      inst.mesh.scale.copy(inst.squashV);
      inst.shell.scale.copy(inst.squashV).multiplyScalar(1.05);

      // Core glow pulse
      if(inst.glow){const g=.5+Math.sin(t*3.5)*.5;inst.glow.material.opacity=.4+g*.3;inst.glow.scale.setScalar(.85+g*.3);}

      // Vertex jiggle (throttled)
      if(throttle){
        const pos=inst.geo.attributes.position;const rest=inst.restPos;const vel=inst.velArr;
        const s=Math.max(...new THREE.Box3().setFromObject(inst.mesh).getSize(new THREE.Vector3()).toArray())*.008;
        for(let i=0;i<pos.count;i++){
          const i3=i*3;
          const rx=rest[i3],ry=rest[i3+1],rz=rest[i3+2];
          const cx=pos.getX(i),cy=pos.getY(i),cz=pos.getZ(i);
          // Spring toward rest
          vel[i3  ]+=(rx-cx)*elastic*.12;vel[i3+1]+=(ry-cy)*elastic*.12;vel[i3+2]+=(rz-cz)*elastic*.12;
          vel[i3  ]*=damp;vel[i3+1]*=damp;vel[i3+2]*=damp;
          // Surface tension noise
          const phase=i*.37;
          vel[i3  ]+=Math.sin(t*freq+phase)*.0006*s;
          vel[i3+1]+=Math.cos(t*freq*.8+phase+1)*.0006*s;
          vel[i3+2]+=Math.sin(t*freq*.7+phase+2)*.0006*s;
          pos.setXYZ(i,cx+vel[i3],cy+vel[i3+1],cz+vel[i3+2]);
        }
        pos.needsUpdate=true;inst.geo.computeVertexNormals();
        inst.shell.geometry.copy(inst.geo);
      }
      window._nexusMarkDirty?.(1);
    });
  }
  ...
  remove(obj){
    const d=this.active.get(obj.uuid);if(!d)return;
    this.scene.remove(d.group);d.group.traverse(c=>{c.geometry?.dispose();c.material?.dispose?.();});
    d.obj.visible=true;this.active.delete(obj.uuid);window._nexusInvalidateBloom?.();
  }
}
...
async function initAddons(){
  ...
  const fireSys=new FireSmokeSimulation(scene);
  const waterSys=new WaterSimulation(scene);
  const sandSys=new SandSimulation(scene);
  const jellySys=new JellySimulation(scene);
  ...
  window._nexusAddons={volSystem,mineralSys,clothSys,goopSys,fireSys,waterSys,sandSys,jellySys,pbrGen,advLight,rtR};
  ...
  // ── Fire/Smoke UI ────────────────────────────────────────────
  document.getElementById('fire-sim-toggle')?.addEventListener('change',e=>{document.getElementById('fire-sim-controls').style.display=e.target.checked?'':'none';});
  wireSlider('fire-sim-intensity','fire-sim-intensity-num',()=>{});wireSlider('fire-sim-height','fire-sim-height-num',()=>{});wireSlider('fire-sim-turb','fire-sim-turb-num',()=>{});
  document.querySelectorAll('#fire-sim-mode .seg-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#fire-sim-mode .seg-btn').forEach(x=>x.classList.remove('seg-active'));b.classList.add('seg-active');}));
  document.getElementById('fire-sim-apply-btn')?.addEventListener('click',()=>{
    const ao=window._nexusGetActive?.();if(!ao||window._nexusIsLight?.(ao))return;
    const mode=document.querySelector('#fire-sim-mode .seg-active')?.dataset.val||'fire';
    fireSys.apply(ao,{mode,intensity:getV('fire-sim-intensity')||1,height:getV('fire-sim-height')||2,turb:getV('fire-sim-turb')||1,color:getS('fire-sim-color')||'#ff4400'});
    document.getElementById('fire-sim-apply-btn').style.display='none';document.getElementById('fire-sim-remove-btn').style.display='';
  });
  document.getElementById('fire-sim-remove-btn')?.addEventListener('click',()=>{const ao=window._nexusGetActive?.();if(!ao)return;fireSys.remove(ao);document.getElementById('fire-sim-apply-btn').style.display='';document.getElementById('fire-sim-remove-btn').style.display='none';});

  // ── Water UI ─────────────────────────────────────────────────
  document.getElementById('water-sim-toggle')?.addEventListener('change',e=>{document.getElementById('water-sim-controls').style.display=e.target.checked?'':'none';});
  wireSlider('water-sim-waveH','water-sim-waveH-num',()=>{});wireSlider('water-sim-speed','water-sim-speed-num',()=>{});wireSlider('water-sim-reflect','water-sim-reflect-num',()=>{});
  document.querySelectorAll('#water-sim-type .seg-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#water-sim-type .seg-btn').forEach(x=>x.classList.remove('seg-active'));b.classList.add('seg-active');}));
  document.getElementById('water-sim-apply-btn')?.addEventListener('click',()=>{
    const ao=window._nexusGetActive?.();if(!ao||window._nexusIsLight?.(ao))return;
    const type=document.querySelector('#water-sim-type .seg-active')?.dataset.val||'water';
    waterSys.apply(ao,{type,waveH:getV('water-sim-waveH')||.18,speed:getV('water-sim-speed')||1,reflect:getV('water-sim-reflect')||.7,color:getS('water-sim-color')||'#0ea5e9'});
    document.getElementById('water-sim-apply-btn').style.display='none';document.getElementById('water-sim-remove-btn').style.display='';
  });
  document.getElementById('water-sim-remove-btn')?.addEventListener('click',()=>{const ao=window._nexusGetActive?.();if(!ao)return;waterSys.remove(ao);document.getElementById('water-sim-apply-btn').style.display='';document.getElementById('water-sim-remove-btn').style.display='none';});

  // ── Sand UI ──────────────────────────────────────────────────
  document.getElementById('sand-sim-toggle')?.addEventListener('change',e=>{document.getElementById('sand-sim-controls').style.display=e.target.checked?'':'none';});
  wireSlider('sand-sim-size','sand-sim-size-num',()=>{});wireSlider('sand-sim-gravity','sand-sim-gravity-num',()=>{});
  document.querySelectorAll('#sand-sim-count-seg .seg-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#sand-sim-count-seg .seg-btn').forEach(x=>x.classList.remove('seg-active'));b.classList.add('seg-active');document.getElementById('sand-sim-count').value=b.dataset.val;}));
  document.getElementById('sand-sim-apply-btn')?.addEventListener('click',()=>{
    const ao=window._nexusGetActive?.();if(!ao||window._nexusIsLight?.(ao))return;
    sandSys.apply(ao,{count:parseInt(document.getElementById('sand-sim-count')?.value)||300,size:getV('sand-sim-size')||.04,gravity:getV('sand-sim-gravity')||9.8,color:getS('sand-sim-color')||'#d4a55a'});
    document.getElementById('sand-sim-apply-btn').style.display='none';document.getElementById('sand-sim-remove-btn').style.display='';
  });
  document.getElementById('sand-sim-remove-btn')?.addEventListener('click',()=>{const ao=window._nexusGetActive?.();if(!ao)return;sandSys.remove(ao);document.getElementById('sand-sim-apply-btn').style.display='';document.getElementById('sand-sim-remove-btn').style.display='none';});

  // ── Jelly UI ─────────────────────────────────────────────────
  document.getElementById('jelly-sim-toggle')?.addEventListener('change',e=>{document.getElementById('jelly-sim-controls').style.display=e.target.checked?'':'none';});
  wireSlider('jelly-sim-elastic','jelly-sim-elastic-num',v=>{const ao=window._nexusGetActive?.();if(ao){const d=jellySys.active.get(ao.uuid);if(d)d.config.elastic=v;}});
  wireSlider('jelly-sim-damp','jelly-sim-damp-num',v=>{const ao=window._nexusGetActive?.();if(ao){const d=jellySys.active.get(ao.uuid);if(d)d.config.damp=v;}});
  wireSlider('jelly-sim-freq','jelly-sim-freq-num',v=>{const ao=window._nexusGetActive?.();if(ao){const d=jellySys.active.get(ao.uuid);if(d)d.config.freq=v;}});
  wireSlider('jelly-sim-opacity','jelly-sim-opacity-num',v=>{const ao=window._nexusGetActive?.();if(ao){const d=jellySys.active.get(ao.uuid);if(d&&d.mesh)d.mesh.material.opacity=v;}});
  document.getElementById('jelly-sim-apply-btn')?.addEventListener('click',()=>{
    const ao=window._nexusGetActive?.();if(!ao||window._nexusIsLight?.(ao))return;
    jellySys.apply(ao,{elastic:getV('jelly-sim-elastic')||.4,damp:getV('jelly-sim-damp')||.92,freq:getV('jelly-sim-freq')||2,color:getS('jelly-sim-color')||'#f472b6',opacity:getV('jelly-sim-opacity')||.75});
    document.getElementById('jelly-sim-apply-btn').style.display='none';document.getElementById('jelly-sim-remove-btn').style.display='';
  });
  document.getElementById('jelly-sim-remove-btn')?.addEventListener('click',()=>{const ao=window._nexusGetActive?.();if(!ao)return;jellySys.remove(ao);document.getElementById('jelly-sim-apply-btn').style.display='';document.getElementById('jelly-sim-remove-btn').style.display='none';});
  ...
  setTimeout(()=>{
    document.querySelector('[data-preset="stone"]')?.classList.add('pbr-selected');
    // Show PBR controls immediately
    const pbrCtrl=document.getElementById('pbr-controls');if(pbrCtrl)pbrCtrl.style.display='';
    const pbrApplyBtn=document.getElementById('pbr-apply-btn');if(pbrApplyBtn)pbrApplyBtn.style.display='none';
    // Show preview
    try{const maps=pbrGen.generate('stone');const prev=document.getElementById('pbr-preview-canvas');if(prev&&maps.albedo?.image){const ctx=prev.getContext('2d');ctx.drawImage(maps.albedo.image,0,0,128,128);}maps.albedo?.dispose();maps.roughness?.dispose();maps.normal?.dispose();maps.ao?.dispose();}catch(e){}
  },800);
  ...
  function refreshPanel(){
    ...
    // PBR: show presets always, but apply btn only when object selected
    const pbrNoSel=document.getElementById('pbr-no-sel');const pbrCtrl=document.getElementById('pbr-controls');const pbrApplyBtn=document.getElementById('pbr-apply-btn');
    if(pbrNoSel)pbrNoSel.style.display=hasObj?'none':'';
    if(pbrCtrl)pbrCtrl.style.display=''; // always visible
    if(pbrApplyBtn)pbrApplyBtn.style.display=hasObj?'':'none';
