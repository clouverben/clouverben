...
export function _serialize() {
  ...
  const animClipEnd   = window.AnimationSystem?.getState?.()?.clipEnd ?? 240;
  const animClips     = window.AnimationSystem?.getState?.()?.clips ?? {};
...
export async function _applyProjectData(data) {
  ...
  setTimeout(() => {
    ...
    AS.clips     = animData.clips || {};
    AS.selectedClipId = null;
    if (animData.fps)      AS.fps = animData.fps;
    if (animData.interp)   AS.interpMode = animData.interp;
    if (animData.duration) AS.clipEnd = animData.duration;
