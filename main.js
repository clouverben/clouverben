...
// Conta vértices totais do modelo e aplica LOD automático no mobile
function autoReduceForMobile(model) {
    ...
    if (totalVerts > 80_000) {
        // Desativa sombras completamente em modelos muito pesados
        model.traverse(child => { if (child.isMesh) { child.castShadow = false; child.receiveShadow = false; } });
        console.log(`[autoReduceForMobile] 🔧 Sombras desativadas (${totalVerts} vértices)`);
    }
    // Bloom sempre ativo, independente do número de vértices
}
