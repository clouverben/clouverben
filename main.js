...
// ── Depth Render Target ──────────────────────────────────────────────────────
const depthRT = new THREE.WebGLRenderTarget(getViewW(), getViewH(), {
...
// ── Post-processing uniforms (shared with shader) ────────────────────────────
const _postU = {
    ...
    cgContrast:         { value: 1.0 },
    cgSaturation:       { value: 1.0 },
...
function syncBloomTexture() { _postU['bloomTexture'].value = bloomComposer.renderTarget2.texture; }
...
// ── Volumetric: project active light to screen space each frame ───────────────
const _volLightWP = new THREE.Vector3();
function _updateVolLightScreen() {
    ...
    for (let i = 0; i < sceneObjects.length; i++) {
        ...
        if (obj.userData?.isLight && obj.userData?.volumetric?.enabled) {
            ...
            _postU.volLightPosScreen.value.set(
                _volLightWP.x * 0.5 + 0.5,
                _volLightWP.y * 0.5 + 0.5,
                _volLightWP.z
            );
            return;
        }
...
function smartRender() {
    if (_bloomCacheDirty) rebuildBloomCache();
    // Always render depth for post effects
...
// model-btn is handled by nexus_helper.js (Nexus Helper panel)
...
window._nexusMarkDirty         = markDirty;
window._nexusGetActive         = () => activeObject;
window._nexusSetActive         = setActiveObject;
