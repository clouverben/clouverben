...
// Expõe globals para nexus-helper.js e outros módulos
window.THREE = THREE;
...
renderer.domElement.addEventListener('pointerup', e => {
    ...
    if (hits.length > 0) {
        ...
        if (activeObject && activeObject !== target) {
            // Clicou em objeto diferente do ativo — adiciona ambos à seleção múltipla
            ...
            // Clicou no objeto já ativo com multi-select — colapsa para seleção única
            _clearMultiSelectOutlines();
            selectedObjects.clear();
            selectedObjects.add(target);
            setActiveObject(target);
        } else {
            // Seleção normal (único objeto)
            _clearMultiSelectOutlines();
            selectedObjects.clear();
            selectedObjects.add(target);
            setActiveObject(target);
...
function _applyMultiSelectOutlines() {
    selectedObjects.forEach(obj => {
        ...
        if (obj.userData._multiSelOutline) return;
...
// Expõe o Set e helpers para nexus-helper.js
...
if (modelBtn)    modelBtn.addEventListener('click',    () => { /* nexus-helper.js */ });
