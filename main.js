...
const VideoExport = (() => {
    ...
    async function _recordMedia(frames, fps, w, h, bitrateMbps, mimeType, ext) {
        ...
        await new Promise(resolve => {
            let idx = 0, deadline = realNow();
            function loop(now) {
                if (idx >= frames.length || cancelled) { recorder.stop(); resolve(); return; }
                if (now >= deadline - 0.5) {
                    ctx.drawImage(frames[idx], 0, 0); frames[idx].close(); idx++;
                    setBar(idx, frames.length); setLb(`Codificando frame ${idx} / ${frames.length}`); updStats(idx, frames.length);
                    deadline += dur; if (deadline < now) deadline = now;
                }
...
async function fixNPOTTextures(model) {
    ...
    for (const tex of textures) {
        ...
        if (!w || !h || (isPOT(w) && isPOT(h))) continue;
        const pw = nextPOT(w), ph = nextPOT(h);
        canvas.width = pw; canvas.height = ph; ctx.drawImage(img, 0, 0, pw, ph);
        const newImg = new Image(pw, ph); newImg.src = canvas.toDataURL();
        await new Promise(r => { newImg.onload = r; newImg.onerror = r; });
        tex.image = newImg; tex.needsUpdate = true; fixed++; await yieldFrame();
    }
    if (fixed > 0) console.log(`[fixNPOTTextures] ✅ ${fixed} texturas POT`);
    return textures;
}
...
async function optimizeModel(model) {
    ...
    for (const [, group] of groups) {
        ...
        try {
            ...
            mergedMesh.castShadow = mergedMesh.receiveShadow = true; mergedMesh.layers.enable(1); mergedMesh.userData.isMerged = true;
...
function cullSmallShadows(model, threshold = 0.05) {
    ...
    model.traverse(child => {
        ...
        if (r < threshold) { child.castShadow = false; culled++; }
    });
...
function deduplicateMaterials(model) {
    ...
    model.traverse(child => {
        ...
        const newMats = mats.map(mat => {
            if (!mat) return mat;
            const key = [mat.type, mat.color?.getHexString()??'', mat.roughness?.toFixed(3)??'', mat.metalness?.toFixed(3)??'', mat.map?.uuid??'', mat.normalMap?.uuid??'', mat.emissive?.getHexString()??'', mat.transparent ? mat.opacity?.toFixed(3) : '1', mat.side].join('|');
            if (canonical.has(key)) { deduped++; return canonical.get(key); }
            canonical.set(key, mat); return mat;
        });
        child.material = Array.isArray(child.material) ? newMats : newMats[0];
    });
    if (deduped > 0) console.log(`[deduplicateMaterials] ✅ ${deduped}`);
}
...
if (modelFileInput) {
    modelFileInput.addEventListener('change', async e => {
        ...
        showImportOverlay('Importando modelo…');
        try {
            if      (name.endsWith('.zip'))                           await loadModelFromZip(file);
            else if (name.endsWith('.gltf') || name.endsWith('.glb')) await loadGltfModel(file);
            else if (name.endsWith('.obj'))                           await loadObjModel(file);
            else { removeImportOverlay(); alert('Use .obj, .gltf, .glb ou .zip'); }
        } catch (err) { console.error('Importação:', err); removeImportOverlay(); alert('Erro: ' + (err.message || err)); }
...
async function loadGltfFromZip(zip, fn, allNames) {
    ...
    return new Promise((ok, err) => {
        new GLTFLoader(mgr).load(main, async gltf => {
...
async function finalizeModelImport(model, originalFileName) {
    ...
    setImportProgress(88, 'Pré-aquecendo GPU…');
...
function updateObjectsList() {
    ...
    document.querySelectorAll('.tree-toggle.has-children').forEach(t => {
        t.addEventListener('click', e => {
            e.stopPropagation();
            const ch = t.parentElement.querySelector('.tree-children');
            if (ch) { const open = ch.style.display !== 'none'; ch.style.display = open ? 'none' : 'block'; t.textContent = open ? '►' : '▼'; const oid = parseInt(t.parentElement.dataset.objectId); if (open) _openGroupIds.delete(oid); else _openGroupIds.add(oid); }
        });
    });

    document.querySelectorAll('.tree-item').forEach(item => {
        const itemId = parseInt(item.dataset.objectId);
        item.addEventListener('mouseenter', () => { if (_reparentSrcId !== null && itemId !== _reparentSrcId) item.classList.add('reparent-hover'); });
        item.addEventListener('mouseleave', () => { item.classList.remove('reparent-hover'); });
        ...
        item.addEventListener('click', e => {
            ...
            if (label) { const obj = findObjectById(itemId); if (obj) { selectBone(null); setActiveObject(obj); updateObjectsList(); } }
        });
        ...
        if (cb) {
            cb.addEventListener('change', e => {
                ...
                if (obj) { if (cb.checked) selectedObjects.add(obj); else selectedObjects.delete(obj); setActiveObject(obj); updateObjectsList(); }
            });
        }
    });
    ...
    document.querySelectorAll('.tree-transform-btn').forEach(btn => {
        btn.addEventListener('click', e => {
            e.stopPropagation();
            showTransformPanel(btn);
        });
    });
...
function setActiveObject(obj) {
    ...
    updateMaterialPanel(); updateLightPanel(); updateParticlePanel(); updateContextButtons(); saveState();
    ...
    ['delete-option','clone-option','rename-option','group-option'].forEach(id => {
        ...
        ne.addEventListener('click', e => {
            ...
            if (id === 'delete-option' && contextMenuTarget) {
                ...
                removeBoneHelpers(); requestShadowUpdate(); invalidateBloomCache(); saveState(); updateObjectsList();
...
if (lightColor) lightColor.addEventListener('input', e => {
    if (activeObject && isLight(activeObject)) {
        ...
        requestShadowUpdate(); markDirty(2); saveState();
    }
});
...
if (lightIntensity && lightIntensityNum) {
    lightIntensity.addEventListener('input', e => { const v=parseFloat(e.target.value); lightIntensityNum.value=v; if(activeObject&&isLight(activeObject)){(activeObject.userData.light||activeObject).intensity=v;saveState();} });
    lightIntensityNum.addEventListener('input', e => { const v=parseFloat(e.target.value); lightIntensity.value=v; if(activeObject&&isLight(activeObject)){(activeObject.userData.light||activeObject).intensity=v;saveState();} });
}
if (lightDistance && lightDistanceNum) {
    lightDistance.addEventListener('input', e => { const v=parseFloat(e.target.value); lightDistanceNum.value=v; if(activeObject&&isLight(activeObject)){const l=activeObject.userData.light||activeObject;if(l.distance!==undefined){l.distance=v;saveState();}} });
    lightDistanceNum.addEventListener('input', e => { const v=parseFloat(e.target.value); lightDistance.value=v; if(activeObject&&isLight(activeObject)){const l=activeObject.userData.light||activeObject;if(l.distance!==undefined){l.distance=v;saveState();}} });
}
...
function applyMaterialChange(prop, val) { if (!activeObject) return; getMeshesFromObject(activeObject).forEach(mesh => { if (!mesh.material) return; const mats = Array.isArray(mesh.material) ? mesh.material : [mesh.material]; mats.forEach(m => { m[prop] = val; m.needsUpdate = true; }); }); saveState(); }
...
if (outlineColor) outlineColor.addEventListener('input', e => { if (activeObject && activeObject.userData.outlineLines) { getMeshesFromObject(activeObject).forEach(m => { if (m.userData.outlineLines) updateOutline(m, true, e.target.value); }); saveState(); } });
...
// ==================== PARTÍCULAS ====================
function updateParticlePanel() { if(!particleNoSelection||!particleControls)return;const isP=activeObject&&isParticleSystem(activeObject);if(isP){particleNoSelection.style.display='none';particleControls.style.display='block';if(particleColor&&activeObject.particleColor)particleColor.value=activeObject.particleColor;if(particleBrightness&&activeObject.brightness!==undefined){particleBrightness.value=activeObject.brightness;if(particleBrightnessNum)particleBrightnessNum.value=activeObject.brightness;}if(particleOpacity&&activeObject.opacity!==undefined){particleOpacity.value=activeObject.opacity;if(particleOpacityNum)particleOpacityNum.value=activeObject.opacity;}}else{particleNoSelection.style.display='block';particleControls.style.display='none';} }
if(particleColor) particleColor.addEventListener('input',e=>{if(activeObject&&typeof activeObject.setColor==='function')activeObject.setColor(e.target.value);saveState();});
if(particleBrightness&&particleBrightnessNum){particleBrightness.addEventListener('input',e=>{const v=parseFloat(e.target.value);particleBrightnessNum.value=v;if(activeObject&&typeof activeObject.setBrightness==='function')activeObject.setBrightness(v);saveState();});particleBrightnessNum.addEventListener('input',e=>{const v=parseFloat(e.target.value);particleBrightness.value=v;if(activeObject&&typeof activeObject.setBrightness==='function')activeObject.setBrightness(v);saveState();});}
if(particleOpacity&&particleOpacityNum){particleOpacity.addEventListener('input',e=>{const v=parseFloat(e.target.value);particleOpacityNum.value=v;if(activeObject&&typeof activeObject.setOpacity==='function')activeObject.setOpacity(v);saveState();});particleOpacityNum.addEventListener('input',e=>{const v=parseFloat(e.target.value);particleOpacity.value=v;if(activeObject&&typeof activeObject.setOpacity==='function')activeObject.setOpacity(v);saveState();});}
...
function animate(timestamp = 0) {
    ...
    if (window.AnimationSystem) {
        ...
        if (povActive && povCamera && window.AnimationSystem.isPlaying()) {
    ...
    if (boneHelpers.length > 0 && (_frameCount & 1) === 0) { updateBoneHelpers(); markDirty(1); }
    if (_shadowPending && sceneObjects.length > 0) { renderer.shadowMap.needsUpdate = true; _shadowPending = false; }
