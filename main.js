...
renderer.domElement.addEventListener('pointerup', e => {
    ...
    if (hits.length > 0) {
        ...
        if (activeObject && activeObject !== target) {
            // Segundo clique em objeto diferente — adiciona os dois à seleção
            ...
            // Clicou no ativo com multi-select — colapsa para um só
...
// (declarado APÓS boneHelpers/BONE_CLICK_PX para evitar TDZ)
function _applyMultiSelectOutlines() {
    selectedObjects.forEach(obj => {
        if (!obj || !obj.isMesh || obj.userData._multiSelOutline) return;
...
ects.length} objeto(s) importado(s)`);
};
