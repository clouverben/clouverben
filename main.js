...
renderer.shadowMap.type       = THREE.PCFShadowMap; // PCFSoft é 2x mais caro com pouco ganho visual
...
const SHADOW_MAP_SIZE = isMobile ? 256 : 1024; // 2048 era 4x mais caro que 1024 sem ganho visível em editor
...
transformControls.addEventListener('change', () => {
    ...
    // ── Bone deform: propaga transforms e atualiza skeleton no GPU ──────────────
    // Sem isso ossos filhos ficam parados e a mesh não deforma ao mover um osso.
    if (selectedBone) {
        // 1. Propaga matrizes: osso movido → filhos → netos
        selectedBone.updateWorldMatrix(true, true);
        // 2. Diz ao Three.js pra repassar as bone matrices pro shader de skinning
        for (let i = 0; i < _skinnedMeshes.length; i++) {
            _skinnedMeshes[i].skeleton.update();
        }
        // 3. Atualiza as esferas/linhas visuais
        updateBoneHelpers();
    }
});
...
const bloomPass = new UnrealBloomPass(
    new THREE.Vector2(Math.round(getViewW() / 2), Math.round(getViewH() / 2)), // metade da res — bloom é borrado, ninguém nota
...
function resizeComposers(w, h) {
    bloomComposer.setSize(Math.round(w / 2), Math.round(h / 2)); // mantém bloom em meia res
...
// Bloom throttle — reutiliza o último resultado quando a cena está parada
// Isso elimina o custo do bloomComposer.render() nos frames idle
let _bloomThrottleCount = 0;
const BLOOM_THROTTLE_IDLE = 4; // frames entre re-renders de bloom quando parado

function renderWithBloom(isDirty) {
    ...
    // Re-renderiza bloom só quando a cena mudou ou a cada N frames (câmera parada)
    _bloomThrottleCount++;
    if (isDirty || _bloomThrottleCount >= BLOOM_THROTTLE_IDLE) {
        darkenNonBloomedCached();
        bloomComposer.render();
        syncBloomTexture();
        restoreMaterialsCached();
        _bloomThrottleCount = 0;
    }
...
function smartRender(isDirty = true) {
    if (_bloomCacheDirty) rebuildBloomCache();
    if (_hasBloomObjects) renderWithBloom(isDirty); else renderer.render(scene, camera);
}
...
// ── Duplo clique no viewport ────────────────────────────────────────────────
let _lastClickTime = 0, _lastClickTargetId = -1;
const DBLCLICK_MS = 380;

// Revela um objeto na árvore de hierarquia: expande grupos-pai, faz scroll e pisca o item
function revealInHierarchy(obj) {
    if (!obj || !objectsListEl) return;
    // Expande todos os grupos ancestrais até a raiz da cena
    let node = obj.parent;
    while (node && node !== scene) {
        _openGroupIds.add(node.id);
        node = node.parent;
    }
    _markObjListDirty();
    updateObjectsList();
    // Aguarda o DOM ser reconstruído antes de fazer scroll + highlight
    requestAnimationFrame(() => {
        const el = objectsListEl.querySelector(`[data-object-id="${obj.id}"]`);
        if (!el) return;
        el.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        el.classList.add('tree-reveal-flash');
        setTimeout(() => el.classList.remove('tree-reveal-flash'), 1400);
    });
}

// CSS para animação de flash na árvore
(function injectRevealCSS() {
    if (document.getElementById('_reveal_css')) return;
    const s = document.createElement('style');
    s.id = '_reveal_css';
    s.textContent = `
        @keyframes _treeFlash {
            0%   { background: rgba(80,180,255,0.35); outline: 1px solid rgba(80,180,255,0.8); }
            60%  { background: rgba(80,180,255,0.15); outline: 1px solid rgba(80,180,255,0.4); }
            100% { background: transparent; outline: none; }
        }
        .tree-reveal-flash {
            animation: _treeFlash 1.4s ease-out forwards !important;
            border-radius: 3px;
        }
    `;
    document.head.appendChild(s);
...
// Encontra o ancestral mais próximo (mais profundo) que esteja em sceneObjects
function _findSceneObjectAncestor(obj) {
    let node = obj;
    while (node) {
        if (sceneObjects.includes(node)) return node;
        node = node.parent;
    }
    return null;
}
...
renderer.domElement.addEventListener('pointerup', e => {
    ...
    if (boneHelpers.length > 0) {
        ...
        if (closest) { activeObject = null; window.activeObject = null; selectedObjects.clear(); selectBone(closest); updateObjectsList(); transformControls.attach(closest); e.stopPropagation(); return; }
    }
    ...
    // ── Candidatos: usa flags userData em vez de layer mask ──────────────────
    // Isso garante que meshes de modelos importados (mesmo sem layer 1 por algum motivo)
    // e meshes dentro de LODs sejam sempre selecionáveis.
    const candidates = [];
    scene.traverse(obj => {
        if (!obj.isMesh || !obj.visible) return;
        // Exclui helpers e objetos de debug
        if (obj.userData.isDefaultLight || obj.userData.isBoneHelper) return;
        if (obj.userData.isFrustumLines  || obj.userData.isFXSprite)   return;
        candidates.push(obj);
    });
    ...
    if (hits.length > 0) {
        ...
        // Encontra o ancestral mais próximo que está em sceneObjects
        // (pode ser o modelo direto, ou um grupo do usuário que o contém)
        let target = _findSceneObjectAncestor(hitMesh);
        if (!target) {
            // Fallback: sobe até filho direto da cena
            let root = hitMesh;
        ...
        const now = Date.now();
        const isDouble = (now - _lastClickTime < DBLCLICK_MS) && (_lastClickTargetId === target.id);
        _lastClickTime = now;
        _lastClickTargetId = target.id;

        if (isDouble) {
            // Duplo clique: encontra o sub-objeto mais relevante para revelar na árvore.
            // Prioridade: (1) modelo importado que contém a mesh, (2) sceneObjects mais próximo
            let revealTarget = null;
            let node = hitMesh;
            while (node && node !== scene) {
                if (node.userData?.isImportedModel) { revealTarget = node; break; }
                if (sceneObjects.includes(node))    { revealTarget = node; break; }
                node = node.parent;
            }
            if (!revealTarget) revealTarget = target;

            // Se o revealTarget não é o activeObject atual, seleciona-o primeiro
            if (revealTarget !== activeObject) {
                selectBone(null); setActiveObject(revealTarget); updateObjectsList();
            }
            revealInHierarchy(revealTarget);
        } else {
            selectBone(null); setActiveObject(target); updateObjectsList();
        }
    } else {
        _lastClickTime = 0; _lastClickTargetId = -1;
...
const _skinnedMeshes = []; // referências a todas as SkinnedMeshes com bones — atualizadas no buildBoneHelpers
...
function buildBoneHelpers(model) {
    ...
    model.traverse(obj => {
        ...
        if (obj.isSkinnedMesh && obj.skeleton) {
            obj.skeleton.bones.forEach(b => bonesSet.add(b));
            // Registra a SkinnedMesh para poder chamar skeleton.update() ao mover ossos
            if (!_skinnedMeshes.includes(obj)) _skinnedMeshes.push(obj);
        }
    });
...
const _boneProj = new THREE.Vector3(); // pré-alocado — evita GC no pointerup
...
function selectBone(bone) {
    ...
    if (bone) {
        ...
        transformControls.attach(bone); window.activeObject = bone; window.selectedBone = bone;
...
function removeBoneHelpersFor(model) {
    ...
    // Remove SkinnedMeshes do registro global
    for (let i = _skinnedMeshes.length - 1; i >= 0; i--) {
        const sm = _skinnedMeshes[i];
        if (sm.skeleton?.bones.some(b => modelBones.has(b))) _skinnedMeshes.splice(i, 1);
    }
...
function isParticleSystem(obj) { return obj && (isFireParticleSystem(obj) || isLaserParticleSystem(obj) || isElectricityParticleSystem(obj) || (obj.userData && obj.userData.isParticle === true)); }
...
if (modelBtn)    modelBtn.addEventListener('click',    () => alert('Modo Modelagem (placeholder)'));
...
(function () {
    ...
    // ── Vetores pré-alocados — zero alocações em hot paths ────────────────────
    // Reutilizados em _nearestEdge, _triVerts, _faceNudge, _highlight*, _pivotCB
    const _tv0    = new THREE.Vector3(); // vértice 0 do triângulo
    const _tv1    = new THREE.Vector3(); // vértice 1
    const _tv2    = new THREE.Vector3(); // vértice 2
    const _tEdgeA = new THREE.Vector3(); // aresta ponto A
    const _tEdgeB = new THREE.Vector3(); // aresta ponto B
    const _tNudge = new THREE.Vector3(); // normal offset para highlight
    const _tCen   = new THREE.Vector3(); // centroide de face/aresta
    const _tSub0  = new THREE.Vector3(); // scratch para cross product
    const _tSub1  = new THREE.Vector3(); // scratch para cross product
    const _tSeg   = new THREE.Line3();   // segmento de aresta para _nearestEdge
    const _tCl    = new THREE.Vector3(); // closest point em _nearestEdge
    const _tLocalPt = new THREE.Vector3(); // ponto de hit em espaço local
    const _tInvMat  = new THREE.Matrix4(); // inversa da matrixWorld
    const _tDelta   = new THREE.Vector3(); // delta de movimento do pivot
    const _tPrevLoc = new THREE.Vector3(); // posição anterior do pivot
    const _tPickP   = new THREE.Vector3(); // ponto temporário em _pickVertex
    const _tVPos    = new THREE.Vector3(); // posição de vértice em _attachPivotVert
    ...
    function _pick(e) {
        ...
        if (selMode === 'face') {
            ...
            if (!hits.length) { _clearSelection(); return; }
            const tri = hits[0].faceIndex;
            // Reutiliza _tInvMat e _tLocalPt — zero alocações
            _tInvMat.copy(editMesh.matrixWorld).invert();
            _tLocalPt.copy(hits[0].point).applyMatrix4(_tInvMat);
            const edge = _nearestEdge(tri, _tLocalPt);
    ...
    function _pickVertex() {
        ...
        let best = -1, bestDist = 0.008;
        for (let i = 0; i < pos.count; i++) {
            _tPickP.set(pos.getX(i), pos.getY(i), pos.getZ(i)).applyMatrix4(editMesh.matrixWorld).project(camera);
            const d = Math.abs(_tPickP.x - mx) + Math.abs(_tPickP.y - my);
    ...
    function _nearestEdge(triIdx, localPt) {
        const pos  = editMesh.geometry.attributes.position;
        const base = triIdx * 3;
        _tv0.set(pos.getX(base),   pos.getY(base),   pos.getZ(base));
        _tv1.set(pos.getX(base+1), pos.getY(base+1), pos.getZ(base+1));
        _tv2.set(pos.getX(base+2), pos.getY(base+2), pos.getZ(base+2));
        const v = [_tv0, _tv1, _tv2];
        let best = 0, bestD = Infinity;
        for (let e = 0; e < 3; e++) {
            _tSeg.set(v[e], v[(e+1)%3]);
            _tSeg.closestPointToPoint(localPt, true, _tCl);
            const d = _tCl.distanceToSquared(localPt);
    ...
    // Preenche _tv0/_tv1/_tv2 com os vértices do triângulo — sem alocar
    function _triVerts(triIdx) {
        const pos = editMesh.geometry.attributes.position, b = triIdx * 3;
        _tv0.set(pos.getX(b),   pos.getY(b),   pos.getZ(b));
        _tv1.set(pos.getX(b+1), pos.getY(b+1), pos.getZ(b+1));
        _tv2.set(pos.getX(b+2), pos.getY(b+2), pos.getZ(b+2));
        return [_tv0, _tv1, _tv2];
    }

    // Calcula normal offset para o highlight — usa _tSub0/_tSub1/_tNudge
    function _faceNudge(vA, vB, vC) {
        _tSub0.subVectors(vB, vA);
        _tSub1.subVectors(vC, vA);
        _tNudge.crossVectors(_tSub0, _tSub1).normalize().multiplyScalar(0.004);
        return _tNudge;
    }
    ...
    function _attachPivotFace(triIdx) {
        const [vA, vB, vC] = _triVerts(triIdx);
        _tCen.set(0,0,0).add(vA).add(vB).add(vC).divideScalar(3);
        _makePivot(_tCen);

        const pos = editMesh.geometry.attributes.position;
        _tPrevLoc.copy(_tCen);

        const faceIndices   = [triIdx*3, triIdx*3+1, triIdx*3+2];
        const sharedIndices = _expandToShared(faceIndices);

        _pivotCB = () => {
            _tDelta.subVectors(_pivot.position, _tPrevLoc);
            _tPrevLoc.copy(_pivot.position);
            for (const i of sharedIndices) {
                pos.setXYZ(i, pos.getX(i)+_tDelta.x, pos.getY(i)+_tDelta.y, pos.getZ(i)+_tDelta.z);
            }
    ...
    function _attachPivotEdge(triIdx, edgeKey) {
        ...
        const [eA, eB] = [[vA,vB],[vB,vC],[vC,vA]][eLocal];
        _tCen.addVectors(eA, eB).multiplyScalar(0.5);
        _makePivot(_tCen);
    ...
    function _attachPivotVert(vertIdx) {
        const pos = editMesh.geometry.attributes.position;
        _tVPos.set(pos.getX(vertIdx), pos.getY(vertIdx), pos.getZ(vertIdx));
        _makePivot(_tVPos);

        _tPrevLoc.copy(_tVPos);
...
document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') {
        ...
        selectBone(null); if (activeObject) transformControls.attach(activeObject);
    }
...
async function finalizeModelImport(model, originalFileName) {
    ...
    // ── Traversal único: faz tudo em uma passagem pela árvore ──────────────────
    // Antes eram 5 traversals separados (setup, material, bones, shadows, bounding).
    // Agora é 1, dividido em chunks async pra não travar o frame.
    const canonical = new Map(); // deduplicação de materiais inline
    let hasBones = false;
    await traverseAsync(model, child => {
        ...
        const newMats = mats.map(mat => {
            ...
            // Deduplicação inline — evita traversal extra
            const key = [
                std.type, std.color?.getHexString() ?? '',
                std.roughness?.toFixed(3) ?? '', std.metalness?.toFixed(3) ?? '',
                std.map?.uuid ?? '', std.normalMap?.uuid ?? '',
                std.roughnessMap?.uuid ?? '', std.metalnessMap?.uuid ?? '',
                std.aoMap?.uuid ?? '', std.emissiveMap?.uuid ?? '',
                std.alphaMap?.uuid ?? '', std.emissive?.getHexString() ?? '',
                std.emissiveIntensity?.toFixed(3) ?? '',
                std.transparent ? std.opacity?.toFixed(3) : '1', std.side,
            ].join('|');
            if (canonical.has(key)) return canonical.get(key);
            canonical.set(key, std); return std;
        });
        child.material = Array.isArray(child.material) ? newMats : newMats[0];

        // ── Bounding volumes + shadow cull ────────────────────
        if (child.geometry) {
            child.geometry.computeBoundingBox();
            child.geometry.computeBoundingSphere();
            // Cull shadows de meshes muito pequenas
            const r = child.geometry.boundingSphere?.radius ?? Infinity;
            const thr = isMobile ? 0.15 : 0.05;
            if (r < thr) child.castShadow = false;
        }
    }, 150); // chunk menor = mais yields = UI mais responsiva durante import

    setImportProgress(72, 'Adicionando à cena…'); await yieldFrame();

    // Mobile: preserva shadow só nas top 20% meshes maiores
    if (isMobile) {
        const meshes = [];
        model.traverse(c => {
            if (c.isMesh && c.geometry?.attributes?.position)
                meshes.push({ mesh: c, verts: c.geometry.attributes.position.count });
        });
        const totalVerts = meshes.reduce((s, m) => s + m.verts, 0);
        if (totalVerts > 50_000) {
            meshes.sort((a, b) => b.verts - a.verts);
            const keep = Math.max(1, Math.ceil(meshes.length * 0.2));
            meshes.forEach(({ mesh }, i) => { if (i >= keep) { mesh.castShadow = false; mesh.receiveShadow = false; } });
        }
    }
    ...
    // Garante layer 1 em TODAS as meshes (incluindo as geradas pelo LOD)
    model.traverse(child => { if (child.isMesh) child.layers.enable(1); });
...
// enterPOV
const _povWPos  = new THREE.Vector3();
const _povWQuat = new THREE.Quaternion();
const _povEul2  = new THREE.Euler(0, 0, 0, 'YXZ');
// doReparent
const _rpWPos     = new THREE.Vector3();
const _rpWQuat    = new THREE.Quaternion();
const _rpWScale   = new THREE.Vector3();
const _rpParentInv = new THREE.Matrix4();
const _rpWorldMat  = new THREE.Matrix4();
const _rpLocalMat  = new THREE.Matrix4();
...
function enterPOV(camObj) {
    ...
    camObj.getWorldPosition(_povWPos);
    camObj.getWorldQuaternion(_povWQuat);

    camera.position.copy(_povWPos);
    camera.quaternion.copy(_povWQuat);

    _povEul2.setFromQuaternion(_povWQuat, 'YXZ');
    povYaw   = _povEul2.y;
    povPitch = _povEul2.x;
...
function doReparent(srcObj, tgtObj) {
    ...
    srcObj.getWorldPosition(_rpWPos); srcObj.getWorldQuaternion(_rpWQuat); srcObj.getWorldScale(_rpWScale);
    srcObj.removeFromParent(); tgtObj.add(srcObj);
    tgtObj.updateMatrixWorld(true);
    _rpParentInv.copy(tgtObj.matrixWorld).invert();
    _rpWorldMat.compose(_rpWPos, _rpWQuat, _rpWScale);
    _rpLocalMat.multiplyMatrices(_rpParentInv, _rpWorldMat);
    _rpLocalMat.decompose(srcObj.position, srcObj.quaternion, srcObj.scale);
    srcObj.updateMatrix();
...
let _objListDirty = true;          // força rebuild na primeira chamada
let _objListActiveId  = -1;        // id do objeto ativo na última renderização
let _objListSceneLen  = -1;        // número de objetos na última renderização
function _markObjListDirty() { _objListDirty = true; }

function updateObjectsList() {
    if (!objectsListEl) return;
    _setupListDelegation();

    // Dirty-check: só reconstrói o HTML se algo realmente mudou
    const activeId  = activeObject?.id ?? -1;
    const sceneLen  = sceneObjects.length;
    if (!_objListDirty && activeId === _objListActiveId && sceneLen === _objListSceneLen) {
        // Apenas atualiza painéis sem mexer no DOM da lista
        updateMaterialPanel(); updateLightPanel(); updateParticlePanel(); updateContextButtons();
        return;
    }
    _objListDirty    = false;
    _objListActiveId = activeId;
    _objListSceneLen = sceneLen;
...
function setActiveObject(obj) {
    ...
    if (obj) {
        selectBone(null); transformControls.attach(obj);
        let _p = obj.parent;
    ...
    _markObjListDirty();
    // updateMaterialPanel/updateLightPanel/updateParticlePanel/updateContextButtons
    // são chamados por updateObjectsList — não duplicar aqui
...
function animate(timestamp = 0) {
    ...
    if (_dirty || _dirtyFrames > 0) { smartRender(_dirty || _dirtyFrames > 1); if (_dirtyFrames > 0) _dirtyFrames--; else _dirty = false; }
}
animate();
