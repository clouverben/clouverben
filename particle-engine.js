...
function _inferStackFromConfig(cfg) {
    const stack = { emitter: null, initializers: [], operators: [], forces: [], renderer: null };
    // Emitter (single-select by cfg.burst)
    stack.emitter = cfg.burst ? 'emit_burst' : 'emit_continuous';
    ...
    stack.renderer = cfg.rendererMode === 'beam' ? 'render_beam' : 'render_sprite';
    return stack;
}
...
export class ParticleSystem {
    constructor(scene, config = {}) {
        ...
        this.stack = { emitter: 'emit_continuous', initializers: [], operators: [], forces: [], renderer: 'render_sprite' };
    ...
    addModule(category, moduleId, values = {}) {
        ...
        if (!mod) return false;
        const list = category === 'emitter' ? null
                   : category === 'renderer' ? null
                   : this.stack[category];

        if (mod.group) {
            // Single-select group (shape / emitMode / renderMode): remove
            // any other module from the same group first.
            if (category === 'emitter')      this.stack.emitter  = moduleId;
            else if (category === 'renderer') this.stack.renderer = moduleId;
            else {
        ...
        this.setConfig({}); // re-sync material/texture/blending from _config
    ...
    removeModule(category, moduleId) {
        const mod = _findModule(category, moduleId);
        if (!mod) return false;
        if (category === 'emitter' || category === 'renderer') return false; // can't remove the only slot, just switch it
    ...
    hasModule(category, moduleId) {
        if (category === 'emitter')  return this.stack.emitter === moduleId;
        if (category === 'renderer') return this.stack.renderer === moduleId;
